...
<body>
		...
		<div style="padding: 3px;" id="toolbar">
			<div style="padding: 3px;" id="searchBar">
				<form id="searchForm">
					...
					<#-- <input id="queryDeviceId" name="deviceId" class="easyui-textbox" label="设备名称:" style="width: 200px;">-->
				...
				<a href="javascript:void(0)" class="easyui-linkbutton btn-primary" data-options="iconCls:'icon-add'" onclick="newPeFactorManual()">新增</a>
				<a href="javascript:void(0)" class="easyui-linkbutton btn-blue" data-options="iconCls:'icon-edit'" onclick="editPeFactorManual()">修改</a>
				<a href="javascript:void(0)" class="easyui-linkbutton btn-red" data-options="iconCls:'icon-remove'" onclick="deletePeFactorManual()">删除</a>


		</div>
		<table id="dg" class="easyui-datagrid" tyle="width: 100%; height: auto;" toolbar="#toolbar" 
				url="${request.contextPath}/factormanual/PeFactorManual/peFactorManualAllList"
				data-options="
					rownumbers:false,
					singleSelect:true,
					striped:true,
			...
			<thead>
				...
				<#--	<th field="deviceId" width="10%">设备名称</th>-->
				   ...
				   <#-- <th field="unitCode" width="5%">单位</th>-->
				  </tr>
			</thead>
		</table>
	</div>
		...
		<form id="fm" method="post" novalidate style="margin: 0; padding: 20px 50px">
			...
			<div style="margin-bottom: 10px">
			 <input class="easyui-combobox"
					name="outputId" id="outputId"  label="点位名称:" required="true"
					data-options="
			...
			<#--<div style="margin-bottom: 10px">
				<input name="deviceId" id="deviceId" class="easyui-textbox" required="true" label="设备ID:" style="width: 100%" >
			</div>-->
			<div style="margin-bottom: 10px">
				<input class="easyui-combobox"
					   name="pluName" id="pluName"  label="因子名称:" required="true"
					   data-options="
					url:'${request.contextPath}/factormanual/PeFactorManual/getComponentFactorManual',
					method:'get',
					valueField:'id',
					textField:'text',
					panelHeight:'auto'
			...
			<div style="margin-bottom: 10px">
				<input name="pluCode" id="pluCode" class="easyui-textbox" required="true" label="因子代码:" style="width: 100%" data-options="events:{blur:jsToUpperCase,keyup:jsToUpperCase}">
			</div>
			<div style="margin-bottom: 10px">
				<input name="upLimit" id="upLimit" class="easyui-textbox" required="true" label="标准值上限:" style="width: 100%" data-options="events:{blur:jsToUpperCase,keyup:jsToUpperCase}">
			</div>
			<div style="margin-bottom: 10px">
				<input name="lowLimit" id="lowLimit" class="easyui-textbox" required="true" label="标准值下限:" style="width: 100%" data-options="events:{blur:jsToUpperCase,keyup:jsToUpperCase}">
			</div>
        	<div style="margin-bottom: 10px">
	...
	<script>
		...
		function doSearch() {
 		...
 		/*		deviceId: $("#queryDeviceId").val().trim()*/
 			});
		}
		function doReset() {
		...
		function newPeFactorManual() {
			...
			url = Ams.ctxPath + '/factormanual/PeFactorManual/savePeFactorManual';
		}
		initPointCombobox();
		//企业下拉框
		...
		function editPeFactorManual() {
			...
			if (row) {
				$.ajax({
					type: 'POST',
					url: Ams.ctxPath + '/factormanual/PeFactorManual/getPeFactorManualByPluEqpId',
		...
		function savePeFactorManual() {
            ...
            if(statusFlag == 'edit'){
                ...
                $.ajax({
                    type: 'POST',
                    url: Ams.ctxPath + '/factormanual/PeFactorManual/isSameFactor',
			...
			if (row) {
				...
				$.messager.confirm("提示信息", "确认删除当前选中的记录吗", function (r) {
					if (r) {
						$.ajax({
							type: 'POST',
							url: Ams.ctxPath + '/factormanual/PeFactorManual/deletePeFactorManualByEqpId',
							data: {'pluEqpId': row.pluEqpId},
							success: function (result) {
								console.log(result);
