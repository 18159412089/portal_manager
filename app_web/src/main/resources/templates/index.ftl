...
<div class="container">
    <div class="item-box">
        ...
        <a class="item" href="${request.contextPath}/environment/rectifition/show?authority=1" target="_blank">
            ...
            <div class="tex">
                <p>突出生态问题统一监管，环保督查任务进度，环境问题整治情况，污染项目整改情况，各环保部门验收情况。</p>
            </div>
        </a>
    ...
    <div class="item-box">
        ...
        <a class="item" href="/env/mainPage/main.do?type=AQI&menu=dataServiceMenu" target="_blank">
            <div class="info-top">
    ...
    <div class="item-box">
        <a class="item" href="${request.contextPath}/jobSchedule/cliqueBuild" target="_blank">
            <div class="info-top">
...
<script type="text/javascript">
    ...
    $(function() {
        $.ajax({
            type: 'POST',
            url: Ams.ctxPath + '/sys/menu/getNewFrontMenu',
            success: function (result) {
                var data = eval(result);
                var htm='<div class="item-box">';
                for(var i=0; i<result.length; i++){
                    if( result.length == 4 && i==2){
                        htm +='</div><div class="item-box">';
                    } else if (result.length == 12 && i == 4) {
                        htm +='</div><div class="item-box">';
                    } else if (result.length>4 && result.length<12 && i==3){
                        htm +='</div><div class="item-box">';
                    }
                    if ((result.length==8|| result.length== 9 || result.length==10) && i==6){
                        htm +='</div><div class="item-box">';
                    } else if(result.length==11 && i==7) {
                        htm +='</div><div class="item-box">';
                    } else if (result.length==12 && i==8) {
                        htm +='</div><div class="item-box">';
                    }

                    htm += '<a class="item" href="' + result[i].url + '" target="_blank">'
                            + '<div class="info-top"><span style="font-size:62px;" class="icon iconcustom '
                            + result[i].icon + '"></span><span>'+ result[i].name + '</span></div>'
                            + '<div class="tex"><p>' + result[i].remark + '</p></div></a>'
                }
                $(".container").html(htm+"</div>");
            },
            dataType: 'json'
        });
    });
