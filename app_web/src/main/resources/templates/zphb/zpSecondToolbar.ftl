...
<div id="pf-hd">
    <span class="pf-logo">
        <img src="${request.contextPath}/static/images/blocks1.png" align="absmiddle"/>  漳浦生态汇聚平台
    </span>
    <div class="pf-user">
...
<script>
    ...
    $(function () {
        // 二级菜单事件
        var second = '<li><a href="${request.contextPath}/zphb/index" target="_self"><i class="icon iconcustom icon-shouye1"></i><span class="title">首页</span></a></li>';
        if($("#pid").val()==1){
            second+="<li class='select-link'><a href='/zphb/enviromonit/water/nationalSurfaceWater?pid=1' target='水环境服务'><i class='icon iconcustom icon-shidu1'></i><span class='title'>水环境服务</span></a></li>"
        }else{
            second+="<li class='select-link'><a href='/zphb/enviromonit/airEnvironment?pid==2' target='大气环境服务'><i class='icon iconcustom icon-huanjing2'></i><span class='title'>大气环境服务</span></a></li>"
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (i, o) {
                    ...
                    second += '<li ' + isSelected + '><a ' + url + ' target="' + target + '"><i class="icon iconcustom ' + o.icon + '"></i><span class="title">' + o.name + '</span></a></li>';
                });
                if($("#pid").val()==1){
                    second+="<li class='select-link'><a href='/zphb/enviromonit/water/nationalSurfaceWater?pid=1' target='水环境服务'><i class='icon iconcustom icon-shidu1'></i><span class='title'>水环境服务</span></a></li>"

                }else{
                    second+="<li class='select-link'><a href='/zphb/enviromonit/airEnvironment?pid==2' target='大气环境服务'><i class='icon iconcustom icon-shidu1'></i><span class='title'>水环境服务</span></a></li>"
        ...
        $(".nav-ul-tag").find("li").mouseover(function () {
            ...
            $(".child-item").find("a").each(function () {
                if (windowPathName.indexOf($(this).attr("href").replace("&authority=1","")) != -1 && $(this).attr("href") != "*") {
                    $(this).parent("li").addClass("select-tag")
                }
            });
        });
    }
