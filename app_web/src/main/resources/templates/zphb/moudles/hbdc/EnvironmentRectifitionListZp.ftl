...
<#--    <a data-options="iconCls:'icon-add'" onclick="earlyone()">-->
<#--        预警时间-->
<#--    </a>-->
    </@sec.authorize>
    </div>
    ...
    <form id="searchForm"  class="searchForm-style">
        <input id="queryProjectName" name="projectId" class="easyui-combobox" style="width:200px;" label="项目:"
               data-options="
            	...
            	valueField:'id',
            	textField:'name'"/>
        <#--<input id="queryAreaCode" label="行政区划:" class="easyui-combobox" name="queryAreaCode"  style="width:200px;" data-options="
...
<#--        <input id="queryLimit" name="queryLimit" class="easyui-datebox yl-easyui-textbox" data-options="editable:false" style="width:160px;">-->
        <label class="control-label">综合条件:</label>
        <input id="queryAreaCode" name="areaCode" class="easyui-textbox" data-options="editable:true,prompt:'输入问题描述/具体要求\模糊查询'" style="width:260px;"/>
    ...
    <div class="details-info-box" id="divTable" >
        <ul class="tit-entry-box" style="display: none" >
...
<#--预警弹窗开始-->
    <#--<div class="early-warning-show" style="display:none;">
        ...
        &lt;#&ndash;<div class="early-head">&ndash;&gt;
        &lt;#&ndash;<span class="icon iconcustom"></span>&ndash;&gt;
        &lt;#&ndash;</div>&ndash;&gt;
    ...
    <div id="dlg" class="easyui-dialog" style="width:600px;height:500px;display: none" data-options="closed:true,modal:true,border:'thin',buttons:'#dlg-buttons'">
        <form id="fm" method="post" novalidate style="margin:0;padding:20px 50px" enctype="multipart/form-data">
            ...
            <div style="margin-bottom:10px">
            	...
            	required:true,
            	url:'${request.contextPath}/zphb/environment/commonRelationTable/listNotPage?code=ENVIRONMEENT_RECTIFITION_04&relation=ENVIRONMEENT_RECTIFITION',
            	valueField:'id',
           ...
           <#-- <div style="margin-bottom:10px">
            ...
            <div style="margin-bottom:10px">
                <input name="areaCode" class="easyui-textbox" data-options="label:'行政区划:',validType:'maxLength[65]'" style="width: 200px;">
            </div>
            <div style="margin-bottom:10px">
                <input name="cityCode" class="easyui-textbox" data-options="label:'所属城市:',validType:'maxLength[65]'" style="width: 200px;">
            </div>
            <div style="margin-bottom:10px">
    ...
    <div id="dlg-numOfRound" class="easyui-dialog" style="width:600px;height:500px;display: none"  data-options="closed:true,modal:true,border:'thin',buttons:'#dlg-closeRoundBtn'">
        <div style="padding-bottom: 10px;padding-top: 10px">
            <form id="numOfRounFm" method="post" novalidate enctype="multipart/form-data" style="display:inline;padding:10px;">
                ...
                <input name="relation" value="NUM_OF_ROUND_ZP" hidden="true"/>
            </form>
...
<#--            </div>-->
    ...
    <#include "/zphb/moudles/hbdc/videoShow.ftl">
    <script>
        var divpage=1;
        var divRowsCount=0;
        ...
        function setRoundData(){
            ...
            $.ajax({
                type: 'POST',
                url: Ams.ctxPath + '/zphb/environment/rectifition/getNumOfRound',
                data: {},
        ...
        function getNextRound(editData){
            $.ajax({
                type: 'POST',
                url: Ams.ctxPath + '/zphb/environment/rectifition/getNextRound',
                data: {num: editData},
                success: function (result) {
                    var newTable = "";
                    if (result == null || result.length == 0 || result == undefined) {
                        // newTable += " <label><input type='radio' name='name' class='check' checked style='position: relative;top: 10px;' value='1'>第1轮</label>";
                        $(".roundid").empty("");
        ...
        function rowTable(data) {
            ...
            for (var i = 0; i<data.rows.length;i++) {
               ...
               /* var warningTime=dataVal.wornTime;
                ...
                if (Ams.isNoEmpty(dataVal.pkid)) {
                    var name = dataVal.picname;
                    name = name.substring(name.lastIndexOf('.')+1,name.length);
                    if(name == 'pdf'||name == 'png'||name == 'jpg'){
                        newRow += "<a  style='color: deepskyblue' href='/environment/waterAttachment/download/" + dataVal.picture + "/2" +
                            "' class='easyui-linkbutton' title='点击查看【"+dataVal.picname+"】附件' target='_blank'><h5>"+dataVal.projectName+"</a>";
                    }else if(name == 'mp4' || name == 'mp3'){
                        newRow += "<a  style='color: deepskyblue' href='javascript:onClick=play(\"" + dataVal.picture + "\")' title='点击查看【"+dataVal.picname+"附件】' class='easyui-linkbutton'><h5>"+dataVal.projectName+"</a>";
                    }else {
                        newRow += "<a style='color: deepskyblue' title='点击查看【"+dataVal.picname+"】附件' href='/debrief/StandingBook/download/" + dataVal.picture +
                            "' class='easyui-linkbutton' target='_blank'><h5 >"+dataVal.projectName+"</a>";
                    }
                }else {
                ...
                newRow += '<p> <span>行政区划</span>'+(Ams.isNoEmpty(dataVal.areaCode)==false?"":dataVal.areaCode) +'</p>';
                newRow += '<p> <span>所属城市</span>'+(Ams.isNoEmpty(dataVal.cityCode)==false?"":dataVal.cityCode) +'</p>';
                newRow += '</div>'
                newRow += '<div class="item">';
               ...
               //  newRow += '<div class="item">';
               //  newRow += ' <p> <span>涉及人员</span>'+ (Ams.isNoEmpty(dataVal.involvePerson)==false?"":dataVal.involvePerson) +'</p>';
               // // newRow += '<p class="color-tag1"> <span class="color-tag1">整改时限</span>'+Ams.stdDateFormat(data
                // Val.timelimit)+'</p>';
               //  newRow += '</div>';
                // newRow += '<div class="item">';
        ...
        function warnTimeComfirm(){
            $.ajax({
                type: 'POST',
                url: Ams.ctxPath + '/zphb/environment/rectifition/setWarnTime',
                data: {
        ...
        function save(){
            ...
            $('#dlg_one #fm_one').form('submit', {
                iframe: false,
                url:  Ams.ctxPath + '/zphb/environment/rectifition/saveRectifition',
                onSubmit: function () {
            ...
            if (uuid) {
                $.messager.confirm("提示信息", "确认删除？", function (r) {
                    if (r) {
                        $.ajax({
                            type: 'POST',
                            url: Ams.ctxPath + '/zphb/environment/rectifition/delete',
                            data: {
                                'uuid': uuid
                            },
                            success: function (result) {
        ...
        function ChangeStatus(rowId) {
            ...
            $.ajax({
                type: 'POST',
                url: Ams.ctxPath + '/zphb/environment/rectifition/get',
                data: {'uuid': rowId},
                success: function (result) {
                    $('#dlg-status').dialog('open').dialog('center').dialog('setTitle', '设置状态');
                    $('#dlg-status #fm').form('load', result.result);
                    url = Ams.ctxPath + '/zphb/environment/rectifition/save';
                },
                dataType: 'json'
            });
        }
        ...
        function openRelation01(relation){
            ...
            if(relation == 'relation01'){
                $('#code').val("ENVIRONMEENT_RECTIFITION_03");
                $('#relation').val("ENVIRONMEENT_RECTIFITION");
                $('#relation-dg').datagrid({
                    url: Ams.ctxPath + '/zphb/environment/commonRelationTable/list',
                    queryParams:{
                        code:'ENVIRONMEENT_RECTIFITION_03',
                ...
                $('#code').val("ENVIRONMEENT_RECTIFITION_04");
                $('#relation').val("ENVIRONMEENT_RECTIFITION");
                $('#relation-dg').datagrid({
                    url: Ams.ctxPath + '/zphb/environment/commonRelationTable/list',
                    queryParams:{
                        code:'ENVIRONMEENT_RECTIFITION_04',
                        relation:'ENVIRONMEENT_RECTIFITION'
                    }
                });
            ...
            $.messager.confirm("提示信息", "如果此前有项目用到此轮数时间，则此次修改将会同步之前项目设置的轮数时间，确认修改？", function (r) {
                if (r) {
                    $('#dlg-update-round #roundFm').form('submit', {
                        url: Ams.ctxPath + '/zphb/environment/commonRelationTable/save',
                        iframe: false,
                        onSubmit: function () {
                            return $(this).form('validate');
                        },
        ...
        function editRelation(uuid){
            ...
            $.ajax({
                type: 'POST',
                url: Ams.ctxPath + '/zphb/environment/commonRelationTable/get',
                data: {'uuid': uuid},
                success: function (result) {
        ...
        function delRelation(uuid){
            $.messager.confirm("提示信息", "确认删除当前选中的记录吗", function (r) {
                if (r) {
                    $.ajax({
                        type: 'POST',
                        url: Ams.ctxPath + '/zphb/environment/commonRelationTable/del',
                        data: {'uuid': uuid},
        ...
        */
         ...
         */
        function getOverTal() {
            var url = Ams.ctxPath + '/zphb/environment/rectifition/wornList'
            var param = {
                "page": 1,
                "rows": 9999,
            }
        ...
        function exportExl(){
            ...
            if ("1" == showAllFlag) {
                window.location.href = "${request.contextPath}/zphb/environment/rectifition/listNew?export=yes&name=" + name + "&limit=" + querylimt + "&status=" + queryStatus;
            } else {
                window.location.href = "${request.contextPath}/zphb/environment/rectifition/wornList?export=yes&name=" + name + "&limit=" + querylimt + "&status=" + queryStatus;
            }
        }
        ...
        function info() {
            ...
            $.ajax({
                type : 'POST',
                url : Ams.ctxPath + '/zphb/environment/rectifition/getCount',
                data : {
        ...
        function getDataShow() {
            ...
            var url;
            if ('1' == showAllFlag) {
                url = Ams.ctxPath + '/zphb/environment/rectifition/listNew'
            }else{
                url = Ams.ctxPath + '/zphb/environment/rectifition/wornList'
            }
