        ...
        <div style="padding:3px;" id="searchBar">
            <form id="searchForm">
                ...
                <input id="queryProjectName" name="name" class="easyui-combobox" style="width:200px;" label=""
                       data-options="
            	required:false,
            	url:'${request.contextPath}/environment/commonRelationTable/listNotPage?code=ENVIRONMEENT_RECTIFITION_03&relation=ENVIRONMEENT_RECTIFITION',
            	valueField:'id',
            	textField:'name'"/>
...
<script>
        ...
        if (row) {
            $.messager.confirm("提示信息", "确认删除当前选中的记录吗", function (r) {
                if (r) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: Ams.ctxPath + '/zphb/enviromonit/distributeMap/delete',
                        data: {
                            'uuid': row.uuid,
    ...
    function doSearch() {
        $('#dg').datagrid({
            url: "/zphb/enviromonit/distributeMap/getFilePage",
            queryParams: {
