...
<script>
    ...
    $.parser.onComplete = function () {
        $("#loadingDiv").fadeOut("normal", function () {
            $(this).remove();
            $('#infoSet').attr("href", "/zphb/enviromonit/distributeMap/distributeMapInfo?pid=" + $("#pid").val() + "&authority=1");
            doSearch();
        });
    };
    ...
    function doSearch() {
        $('#dg').datagrid({
            url: "/zphb/enviromonit/distributeMap/getListPage",
            queryParams: {
    ...
    function trace() {
        ...
        $.ajax({
            type: 'POST',
            url: '/zphb/enviromonit/distributeMap',
...
//       console.log( cl_n.parent().find(".change-line").length)
    ...
    function getEnvironmentProjectRoundInfo() {//获取环保督察项目轮数信息
        $.ajax({
            url: Ams.ctxPath + '/zphb/enviromonit/distributeMap/getRoundList',
            data: {},
            type: 'POST',
