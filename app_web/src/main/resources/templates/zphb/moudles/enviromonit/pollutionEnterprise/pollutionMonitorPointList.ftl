...
<body>
			...
			<div style="padding: 3px;" id="searchBar">
				<form id="searchForm">
						<div class="inline-block">
							...
							<select id="queryPeId" name="queryPeId" class="easyui-combobox" style="width: 200px;"></select>
						</div>
			 	...
			 	<a href="#" class="easyui-linkbutton btn-sky-blue" data-options="iconCls:'icon-add'" onclick="newPeMonitorPoint()">新增</a>
			    <a href="#" class="easyui-linkbutton btn-orange" data-options="iconCls:'icon-edit'" onclick="editPeMonitorPoint()">修改</a>
		  		<a href="#" class="easyui-linkbutton btn-red" data-options="iconCls:'icon-remove'" onclick="deletePeMonitorPoint()">删除</a>

		</div>
		<table id="dg" class="easyui-datagrid" tyle="width: 100%; height: auto;" toolbar="#toolbar" 
				url="${request.contextPath}/zphb/monitorPoint/pemonitorpoint/peMonitorPointList"
				data-options="
					rownumbers:false,
		            singleSelect:true,
		            striped:true,
			...
			<thead>
				<tr>
							...
							<th field="peName" formatter="getEnterpriseName" width="10">企业名称</th>
		...
		<form id="fm" method="post" novalidate >
			<input name="uuid" hidden="true" /> 
					...
					<div style="margin-bottom: 10px">
						<label class="textbox-label textbox-label-before" title="排污口编号">排污口编号:</label>
								<input id="code" required="required" name="code" class="easyui-textbox" data-options="validType:'maxLength[16]'" style="width: 200px;">
					</div>
					<div style="margin-bottom: 10px">
						<label class="textbox-label textbox-label-before" title="排污口名称">排污口名称:</label>
				...
				<#--	<div style="margin-bottom: 10px">
					...
					<div style="margin-bottom: 10px">
						<label class="textbox-label textbox-label-before" title="企业ID">企业:</label>
						<select id="peId"  required="required" name="peId" class="easyui-combobox" style="width: 200px;"></select>
					</div>
					<div class = "hide" style="margin-bottom: 10px">
	...
	<script>
		...
		function getEnterpriseName(value,row,index){
			if(row.zpPeEnterpriseData != null){
				return row.zpPeEnterpriseData.peName;
			}else{
				return value ;
			}
		}
		...
		function doSearch() {
			...
			$('#dg').datagrid('load', {
				...
				peId: $("#queryPeId").combobox("getValue").trim(),
		...
		function newPeMonitorPoint() {
			$('#dlg').dialog('open').dialog('center').dialog('setTitle', '新增监测站点');
			$('#fm').form('clear');
			$('#code').textbox({readonly: false});
			url = Ams.ctxPath + '/zphb/monitorPoint/pemonitorpoint/savePeMonitorPoint';
		}
		...
		function editPeMonitorPoint() {
			...
			if (row) {
				$.ajax({
					type: 'POST',
					url: Ams.ctxPath + '/zphb/monitorPoint/pemonitorpoint/getPeMonitorPoint',
					data: {'uuid': row.outputId},
					success: function (result) {
						$('#dlg').dialog('open').dialog('center').dialog('setTitle', '修改监测站点');
			...
			if (row) {
				$.messager.confirm("提示信息", "确认删除当前选中的记录吗", function (r) {
					if (r) {
						$.ajax({
							type: 'POST',
							url: Ams.ctxPath + '/zphb/monitorPoint/pemonitorpoint/deletePeMonitorPoint',
							data: {'uuid': row.outputId},
							success: function (result) {
