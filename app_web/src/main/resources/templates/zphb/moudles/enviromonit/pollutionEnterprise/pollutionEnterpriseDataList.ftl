...
<body>
	...
	<div class="easyui-layout" fit=true>
			...
			<div style="padding: 3px;" id="searchBar">
				<form id="searchForm">
						<div class="inline-block">
							<label class="textbox-label textbox-label-before" title="污染源企业名称">污染源企业名称:</label>
		...
		<table id="dg" class="easyui-datagrid" tyle="width: 100%; height: auto;" toolbar="#toolbar" 
				url="${request.contextPath}/zphb/enterprise/peenterprisedata/pollutionEnterpriseDataList"
				data-options="
					rownumbers:false,
		            singleSelect:true,
		            striped:true,
			...
			<thead>
				<tr>
							<th field="peCode" width="10">污染源企业编号</th>
							<th field="peName" width="10">污染源企业名称</th>
					        <th field="unitTypeCode" width="10">单位类别编码</th>
							<th field="sizeCode" width="10">企业规模编码</th>
							<th field="tradeCode" width="10">行业类别</th>
					        <th field="longValue" width="10">经度</th>
					        <th field="latValue" width="10">纬度</th>
							<th field="address" width="10">地址</th>
					        <th field="tel" width="10">电话</th>
					        <th field="envPrincipal" width="10">责任人</th>
				</tr>
			</thead>
		</table>
	</div>
		...
		<form id="fm" method="post" novalidate >
			<input name="uuid" hidden="true" /> 
					<div  class="hide"  style="margin-bottom: 10px">
						<label class="textbox-label textbox-label-before" title="污染源企业主键">污染源企业主键:</label>
								<input id="peId" name="peId" class="easyui-textbox" data-options="validType:'number'" style="width: 200px;">
					</div>
					<div style="margin-bottom: 10px">
						<label class="textbox-label textbox-label-before" title="污染源企业编号">污染源企业编号:</label>
	...
	<script>
		...
		function newPeEnterpriseData() {
			$('#dlg').dialog('open').dialog('center').dialog('setTitle', '新增污染源企业自动监控企业信息表');
			$('#fm').form('clear');
			$('#code').textbox({readonly: false});
			url = Ams.ctxPath + '/zphb/enterprise/peenterprisedata/savePeEnterpriseData';
		}
		...
		function editPeEnterpriseData() {
			...
			if (row) {
				$.ajax({
					type: 'POST',
					url: Ams.ctxPath + '/zphb/enterprise/peenterprisedata/getPeEnterpriseData',
					data: {'uuid': row.peId},
					success: function (result) {
						$('#dlg').dialog('open').dialog('center').dialog('setTitle', '修改污染源企业自动监控企业信息表');
			...
			if (row) {
				$.messager.confirm("提示信息", "确认删除当前选中的记录吗", function (r) {
					if (r) {
						$.ajax({
							type: 'POST',
							url: Ams.ctxPath + '/zphb/enterprise/peenterprisedata/deletePeEnterpriseData',
							data: {'uuid':row.peId},
							success: function (result) {
