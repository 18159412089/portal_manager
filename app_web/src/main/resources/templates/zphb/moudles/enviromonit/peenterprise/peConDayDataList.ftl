	        ...
	        <div  id="searchBar" class="searchBar">
	            <form id="searchForm">
	                <div class="inline-block">
                        ...
                        <input id="queryMeasureStartTime" name="queryMeasureStartTime" class="easyui-datetimebox" data-options="editable:false" style="width:300px;">
					</div>
                    <div class="inline-block">
                        <label>- </label>
                        <input id="queryMeasureEndTime" name="queryMeasureEndTime" class="easyui-datetimebox" data-options="editable:false"  style="width:220px;">
					</div>
...
<script>
	...
	function showMonitorDeviceData(record) {
        var queryMeaseStartTime = $("#queryMeasureStartTime").val();
        var queryMeasureEndTime = $("#queryMeasureEndTime").val();
		...
		getPeConDayDataListByOutputId(outputId,queryMeaseStartTime,queryMeasureEndTime,page,pageSize);
	}
			...
			success : function(data) {
				...
				$('#monitorDeviceDataTable').datagrid('getPager').pagination({
					onSelectPage:function(pageNumber, pageSize){
				        var queryMeaseStartTime = $("#queryMeasureStartTime").val();
				        var queryMeasureEndTime = $("#queryMeasureEndTime").val();
						getPeConDayDataListByOutputId(outputId,queryMeaseStartTime,queryMeasureEndTime,pageNumber,pageSize);
					}
				});
            }
		});
	...
	//根据排口获取天监测数据
	function getPeConDayDataListByOutputId(outputId,queryMeaseStartTime,queryMeasureEndTime, page, pageSize) {
		if (outputId == null){
			return;
		}
    ...
    function exportData() {
        ...
        var page = 1;
        var pageSize = 20;
        window.open(Ams.ctxPath + '/dayData/pecondaydata/export?queryMeasureStartTime='+queryMeasureStartTime+'&queryMeasureEndTime='+queryMeasureEndTime+
