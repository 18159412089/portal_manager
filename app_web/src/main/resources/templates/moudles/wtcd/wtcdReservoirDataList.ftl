...
<body>
	...
	<div class="easyui-layout" fit=true>
		<div  id="toolbar">
			...
			<@sec.authorize access= "hasRole('ROLE_ROOT') OR hasRole('ROLE_ADMIN')" > 
				<a href="javascript:void(0)" class="easyui-linkbutton btn-primary" data-options="iconCls:'icon-add'" onclick="newWtcdReservoirData()">新增</a>
				<a href="javascript:void(0)" class="easyui-linkbutton btn-blue" data-options="iconCls:'icon-edit'" onclick="editWtcdReservoirData()">修改</a>
				<a href="javascript:void(0)" class="easyui-linkbutton btn-red" data-options="iconCls:'icon-remove'" onclick="deleteWtcdReservoirData()">删除</a>
			</@sec.authorize>
			 -->
		</div>
		<table id="dg" class="easyui-datagrid" tyle="width: 100%; height: auto;" toolbar="#toolbar" 
				url="${request.contextPath}/wtcd/wtcdReservoirData/wtcdReservoirDataList"
				data-options="
					rownumbers:false,
					singleSelect:true,
					striped:true,
			...
			<thead>
				<tr>
					...
					<th formatter="Ams.tooltipFormat" field="w" width="10%">蓄水量(百万立方米)</th>
					<th formatter="Ams.tooltipFormat" field="zi" width="10%">水位(米)</th>
					<th formatter="Ams.tooltipFormat" field="rksj" width="20%">入库时间</th>
					<th formatter="Ams.tooltipFormat" field="updatetimeRjwa" width="20%">更新时间</th>
					<th formatter="Ams.tooltipFormat" field="ymdhm" width="20%">时间</th>
				</tr>
			</thead>
		</table>
	</div>
	...
	<div id="dlg" class="easyui-dialog" style="width: 400px"
		...
		<form id="fm" method="post" novalidate style="margin: 0; padding: 20px 50px">
			...
			<div style="margin-bottom: 10px">
				<input name="W" id="W" class="easyui-textbox" required="true" label="蓄水量  百万立方米:" style="width: 100%" data-options="events:{blur:jsToUpperCase,keyup:jsToUpperCase}">
			</div>
			<div style="margin-bottom: 10px">
				<input name="RKSJ" id="RKSJ" class="easyui-textbox" required="true" label="入库时间:" style="width: 100%" data-options="events:{blur:jsToUpperCase,keyup:jsToUpperCase}">
			</div>
			<div style="margin-bottom: 10px">
			...
			<div style="margin-bottom: 10px">
				<input name="YMDHM" id="YMDHM" class="easyui-textbox" required="true" label="时间:" style="width: 100%" data-options="events:{blur:jsToUpperCase,keyup:jsToUpperCase}">
			</div>
			<div style="margin-bottom: 10px">
				<input name="ZI" id="ZI" class="easyui-textbox" required="true" label="水位  米:" style="width: 100%" data-options="events:{blur:jsToUpperCase,keyup:jsToUpperCase}">
			</div>
		</form>
	</div>

	<div id="dlg-buttons">
		<a href="javascript:void(0)" class="easyui-linkbutton c6" iconCls="icon-ok" onclick="saveWtcdReservoirData()" style="width: 90px">保存</a>
	...
	<script>
		...
		function newWtcdReservoirData() {
			$('#dlg').dialog('open').dialog('center').dialog('setTitle', '新增水利厅水文站点');
			$('#fm').form('clear');
			$('#code').textbox({readonly: false});
			url = Ams.ctxPath + '/wtcd/wtcdReservoirData/saveWtcdReservoirData';
		}

		function editWtcdReservoirData() {
			$('#fm').form('clear');
			var row = $('#dg').datagrid('getSelected');
			if (row) {
				$.ajax({
					type: 'POST',
					url: Ams.ctxPath + '/wtcd/wtcdReservoirData/getWtcdReservoirData',
					data: {'uuid': row.uuid},
		...
		function saveWtcdReservoirData() {
			$('#fm').form('submit', {
				url: url,
				onSubmit: function () {
		...
		function deleteWtcdReservoirData() {
			var row = $('#dg').datagrid('getSelected');
			if (row) {
				$.messager.confirm("提示信息", "确认删除当前选中的记录吗", function (r) {
					if (r) {
						$.ajax({
							type: 'POST',
							url: Ams.ctxPath + '/wtcd/wtcdReservoirData/deleteWtcdReservoirData',
							data: {'uuid': row.uuid},
							success: function (result) {
		...
		function exportData() {
			doSearch();
			window.open(Ams.ctxPath + '/wtcd/wtcdReservoirData/getListExport' +
					'?STCD='+$("#querySTCD").val().trim());
		}
</script>
</body>
