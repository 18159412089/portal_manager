...
<body>
	...
	<div class="easyui-layout" fit=true>
		<div id="toolbar">
			...
			<@sec.authorize access= "hasRole('ROLE_ROOT') OR hasRole('ROLE_ADMIN')" > 
				<a href="javascript:void(0)" class="easyui-linkbutton btn-primary" data-options="iconCls:'icon-add'" onclick="newWtcdRiverwayData()">新增</a>
				<a href="javascript:void(0)" class="easyui-linkbutton btn-blue" data-options="iconCls:'icon-edit'" onclick="editWtcdRiverwayData()">修改</a>
				<a href="javascript:void(0)" class="easyui-linkbutton btn-red" data-options="iconCls:'icon-remove'" onclick="deleteWtcdRiverwayData()">删除</a>
			</@sec.authorize>
			 -->
		</div>
		<table id="dg" class="easyui-datagrid" tyle="width: 100%; height: auto;" toolbar="#toolbar" 
				url="${request.contextPath}/wtcd/wtcdRiverwayData/wtcdRiverwayDataList"
				data-options="
					rownumbers:false,
					singleSelect:true,
					striped:true,
			...
			<thead>
				<tr>
					...
					<th formatter="Ams.tooltipFormat" field="q" width="10%">流量(立方米)</th>
					<th formatter="Ams.tooltipFormat" field="zr" width="10%">水位(米)</th>
					<th field="rksj" width="20%" formatter="Ams.timeDateFormat">入库时间</th>
					<th field="updatetimeRjwa" width="20%" >更新时间</th>
	...
	<div id="dlg" class="easyui-dialog" style="width: 400px"
		...
		<form id="fm" method="post" novalidate style="margin: 0; padding: 20px 50px">
			...
			<div style="margin-bottom: 10px">
				<input name="Q" id="Q" class="easyui-textbox" required="true" label="流量   立方米:" style="width: 100%" data-options="events:{blur:jsToUpperCase,keyup:jsToUpperCase}">
			</div>
			<div style="margin-bottom: 10px">
			...
			<div style="margin-bottom: 10px">
				<input name="ZR" id="ZR" class="easyui-textbox" required="true" label="水位  米:" style="width: 100%" data-options="events:{blur:jsToUpperCase,keyup:jsToUpperCase}">
			</div>
		</form>
	</div>

	<div id="dlg-buttons">
		<a href="javascript:void(0)" class="easyui-linkbutton c6" iconCls="icon-ok" onclick="saveWtcdRiverwayData()" style="width: 90px">保存</a>
	...
	<script>
		...
		function newWtcdRiverwayData() {
			$('#dlg').dialog('open').dialog('center').dialog('setTitle', '新增水利厅水文站点');
			$('#fm').form('clear');
			$('#code').textbox({readonly: false});
			url = Ams.ctxPath + '/wtcd/wtcdRiverwayData/saveWtcdRiverwayData';
		}

		function editWtcdRiverwayData() {
			$('#fm').form('clear');
			var row = $('#dg').datagrid('getSelected');
			if (row) {
				$.ajax({
					type: 'POST',
					url: Ams.ctxPath + '/wtcd/wtcdRiverwayData/getWtcdRiverwayData',
					data: {'uuid': row.uuid},
		...
		function deleteWtcdRiverwayData() {
			var row = $('#dg').datagrid('getSelected');
			if (row) {
				$.messager.confirm("提示信息", "确认删除当前选中的记录吗", function (r) {
					if (r) {
						$.ajax({
							type: 'POST',
							url: Ams.ctxPath + '/wtcd/wtcdRiverwayData/deleteWtcdRiverwayData',
							data: {'uuid': row.uuid},
							success: function (result) {
		...
		function exportData() {
			doSearch();
			window.open(Ams.ctxPath + '/wtcd/wtcdRiverwayData/getListExport' +
					'?STCD='+$("#querySTCD").val().trim());
		}
</script>
</body>
