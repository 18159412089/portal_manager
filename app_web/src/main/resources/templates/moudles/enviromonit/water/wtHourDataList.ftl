...
<script>
    ...
    function doSearch() {
    	...
    	if(isNoEmpty(startTime)&&isNoEmpty(endTime)){
    		$('#dg').datagrid({
    			url: '${request.contextPath}/enviromonit/water/wtHourData/getPageList',
    		    queryParams: {
    ...
    function doReset() {
        ...
        $('#startTime').datebox().datebox('setValue',getNowDate(6).substring(0,10));
        $('#endTime').datebox().datebox('calendar').calendar({
            validator: function (date) {
		...
		$('#endTime').datebox().datebox('setValue',getNowDate(0).substring(0,10));
        doSearch();
    }
  ...
  //获取时间格式化(cutDay为往前几天，0为当天)
    ...
    function exportData() {
        ...
        if(isNoEmpty(startTime)&&isNoEmpty(endTime)){
            ...
            window.open(Ams.ctxPath + '/enviromonit/water/wtHourData/export?pointName='+pointName+'&startTime='+startTime+' 00'+'&endTime='+endTime+' 23');
        }
    }
</script>
</body>
