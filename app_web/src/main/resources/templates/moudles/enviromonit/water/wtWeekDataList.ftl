...
<div class="easyui-layout" fit=true>
    ...
    <table id="dg" class="easyui-datagrid" style="width:100%;height:auto;" toolbar="#toolbar"
        ...
        <tr>
            ...
            <th field="weekNumber" width="120px" formatter="Ams.tooltipFormat">周</th>
...
<script>
	...
	$(function(){
		...
		$('#startTime').datebox('setValue',getNowDate(27).substring(0,10));
	...
	$('#endTime').datebox().datebox('calendar').calendar({
		 validator: function(date){
				var now = new Date();
				var d2 = new Date(getNowDate(28).substring(0,10));
				var d1 = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    ...
    function doSearch() {
    	...
    	if(isNoEmpty(startTime)&&isNoEmpty(endTime)){
    		$('#dg').datagrid({
    			url: '${request.contextPath}/enviromonit/water/wtWeekData/getPageList',
    		    queryParams: {
    ...
    function doReset() {
        ...
        $('#endTime').datebox().datebox('calendar').calendar({
            validator: function (date) {
                var d1 = new Date(getNowDate(28).substring(0,10));
                var d3 = new Date();
                return d1 <= date && date <= d3;
            }
        });
  ...
  //获取时间格式化(cutDay为往前几天，0为当天)
    ...
    function exportData() {
        ...
        if(isNoEmpty(startTime)&&isNoEmpty(endTime)){
            ...
            window.open(Ams.ctxPath + '/enviromonit/water/wtWeekData/export?pointName='+pointName+'&startTime='+startTime+'&endTime='+endTime);
    }
</script>
</body>
</html>