    ...
    <div id="searchBar1" class="searchBar">
        <form id="searchForm1">
            ...
            <div class="inline-block fr">
                <!-- 单选按钮组 -->
                <div id="dayYear" class="radio-button-group style-btn-group">
...
<script>
    ...
    var url = Ams.ctxPath + "/enviromonit/airDataService/getPassYearAnalysisAirMorePoint";
    var idx = '';
    ...
    function doSearch() {
        ...
        if (!Ams.isNoEmpty(factor)||!Ams.isNoEmpty(startTime)) {
            ...
            $.ajax({
                type: "post",
                url: Ams.ctxPath + "/enviromonit/airDataService/getPassYearAnalysisAirMorePoint",
                data: {
                    polluteName: factor,
    ...
    function setCompareData(data, rq) {
        ...
        if (getYearDay() == '月') {
            ...
            $('#title' + idx).text(curYear + '年' + rq.split("-")[0] + '月' + rq.split("-")[1] + '日');
            $('#title1' + idx).text(startTime + '年' + rq.split("-")[0] + '月' + rq.split("-")[1] + '日');
            $('#title2' + idx).text($('#title' + idx).text() + '与' + $('#title1' + idx).text() + '同比');
        }
        // 当前年
