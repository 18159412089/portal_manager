...
<div id="toolbar">
    <div style="padding:3px;" id="searchBar">
        <form id="searchForm">
            <input id="queryCode" class="easyui-textbox" label="供应商编号:" style="width:200px;">
            <input id="queryName" class="easyui-textbox" label="供应商名称:" style="width:200px;">
            <input id="queryPhone" class="easyui-textbox" label="供应商电话:" style="width:200px;">
...
<!-- Vendor datagrid -->
<table id="dg" class="easyui-datagrid" title="供应商管理" style="width:100%px;height:auto"
       url="${request.contextPath}/crm/vendor/list" toolbar="#toolbar"
       data-options="
				    rownumbers:false,
				    singleSelect:true,
				    autoRowHeight:false,
...
<div id="dlg" class="easyui-dialog" style="width: 400px"
    ...
    <form id="fm" method="post" novalidate style="margin: 0; padding: 20px 50px">
        ...
        <div style="margin-bottom: 10px">
            <input name="code" class="easyui-textbox" required="true" label="供应商编号:" style="width: 100%">
        </div>
        <div style="margin-bottom: 10px">
            <input name="name" class="easyui-textbox" required="true" label="供应商名称:" style="width: 100%">
        </div>
        <div style="margin-bottom: 10px">
...
<div id="dlg-buttons">
    <a href="javascript:void(0)" class="easyui-linkbutton c6" iconCls="icon-ok" onclick="saveVendor()"
...
<script>
    ...
    function newVendor() {
        ...
        $('#fm').form('clear');
        url = Ams.ctxPath + '/crm/vendor/save';
    }
    ...
    function editVendor() {
        ...
        if (row) {
            $.ajax({
                type: 'POST',
                url: Ams.ctxPath + '/crm/vendor/get',
                data: {'uuid': row.uuid},
                success: function (result) {
                    $('#dlg').dialog('open').dialog('center').dialog('setTitle', '修改供应商');
                    $('#fm').form('load', result.crmVendor);
                    url = Ams.ctxPath + '/crm/vendor/save';
                },
                dataType: 'json'
            });
        ...
        if (row) {
            $.messager.confirm("提示信息", "确认删除当前选中的记录吗", function (r) {
                if (r) {
                    $.ajax({
                        type: 'POST',
                        url: Ams.ctxPath + '/crm/vendor/del',
                        data: {
                            'uuid': row.uuid,
                            'userid': row.userid
                        },
    ...
    function newAdmin() {
        ...
        if (row) {
            ...
            url = Ams.ctxPath + '/crm/admin/save?vendorId=' + row.uuid;
        } else {
    ...
    function saveAdmin() {
        $('#adminfm').form('submit', {
            url: url,
            onSubmit: function () {
                $('#logintype').textbox('setValue', 'SUPPLIER');
                return $(this).form('validate');
            },
            success: function (result) {
                var result = JSON.parse(result);
    ...
    function removeAdmin() {
        ...
        if (row) {
            ...
            $.messager.confirm("提示信息", "确认删除当前供应商的管理员吗", function (r) {
                if (r) {
                    $.ajax({
                        type: 'POST',
