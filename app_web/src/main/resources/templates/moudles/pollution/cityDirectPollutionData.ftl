...
<div class="easyui-layout" fit=true >
    ...
    <table id="dg" class="easyui-datagrid" style="width:100%;height:90%;" toolbar="#toolbar"
        ...
        <thead>
        <tr>
            <th width="60" rowspan="2" formatter="Ams.tooltipFormat" field="dw" >部门</th>
...
<div id="dlg" class="easyui-dialog" style="width:990px"
    ...
    <form id="fm" method="post" novalidate style="margin:0;padding:20px 50px" enctype="multipart/form-data">
        <table class="table insp-table">
            ...
            <tr>
                <td class="title tr">市直部门</td>
                <td class="con">
                    <input name="dw" data-options="validType:'maxLength[2550]'" id="regionName" class="easyui-textbox"
                ...
                <td class="con">
                    <input class="easyui-combobox" name="wryzl" value="${pointCode!}" id="regionName"
                           data-options="
									url:'${request.contextPath}/env/pollution2/data/getType?flag=wryzl&tblName=CITY_DIRECT_POLLUTION_DATA',
									method:'post',
									editable:false,
									valueField:'name',
						            textField:'name',
            ...
            <tr>
                ...
                <td class="con">
                    <input class="easyui-combobox" name="wrylx" value="${pointCode!}" id="regionName"
                           data-options="
									url:'${request.contextPath}/env/pollution2/data/getType?flag=wrylx&tblName=CITY_DIRECT_POLLUTION_DATA',
									method:'post',
									editable:false,
									valueField:'name',
						            textField:'name',
...
<script>
    ...
    function newInfo() {
        ...
        url = Ams.ctxPath + '/env/pollution2/cityDirect/saveInfo';
    }
        ...
        if (row) {
            $.messager.confirm("提示信息", "确认删除当前选中的记录吗", function (r) {
                if (r) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: Ams.ctxPath + '/env/pollution2/cityDirect/deleteInfo',
                        data: {
                            'pkid': row.uuid
                        },
                        beforeSend: function () {
            ...
            $.messager.confirm("提示信息", "确认导入文件：<br/>"+newVal+"？", function (r) {
                if (r) {
                    ...
                    $('#uplodFile').form('submit', {
                        url: Ams.ctxPath + "/env/pollution2/cityDirect/importFile",
                        onSubmit: function () {
                            var isValid = $(this).form('validate');
                            if (!isValid){
                                $.messager.progress('close');	// hide progress bar while the form is invalid
    ...
    function downloadFile(){
        ...
        $eleForm.attr("action","${request.contextPath}/static/js/moudles/pollution/templates/市直污染源数据录入模板.xls");
