...
<div class="easyui-layout" fit=true style="padding-left:10px;padding-right:10px;">
        ...
        <div style="padding:3px;" id="searchBar">
            <form id="searchForm">
                ...
                <input class="easyui-combobox" name="queryDeptName" id="queryDeptName"  data-options="
						url:'/jobSchedule/jobScheduleDepartment/getDeptList?category=VEHICLE',
						editable:false,
						method:'get',
						valueField:'id',
						textField:'text',
    ...
    <table id="dg" class="easyui-datagrid" style="width:100%;height:auto;" toolbar="#toolbar"
           url="${request.contextPath}/jobSchedule/vehiclePollution/list" data-options="
            queryParams : '',
            rownumbers:true,
            singleSelect:true,
            striped:true,
...
<div id="dlg" class="easyui-dialog" style="width:500px" data-options="closed:true,modal:true,border:'thin',buttons:'#dlg-buttons'">
    <form id="fm" method="post" novalidate style="margin:0;padding:20px 50px" enctype="multipart/form-data">
        ...
        <div style="margin-bottom:10px">
        	<input class="easyui-combobox" required="true" name="departmentId" id="departmentId" label="部门名称:" required="true" data-options="
						url:'/jobSchedule/jobScheduleDepartment/getDeptList?category=VEHICLE',
						editable:false,
						method:'get',
						valueField:'id',
...
<script>
    ...
    function formatView(value,row,index){
    	...
    	if(row.type == "mp4"){
			...
			return "<div>"+Ams.setImageSee()+"<a href='/jobSchedule/vehiclePollution/viewFile/"+row.mongoid+"/"+row.type
    ...
    function play(mongoid){
    	...
    	$('#video').attr("src", Ams.ctxPath+'/jobSchedule/vehiclePollution/viewFile/'+mongoid+'/mp4');
    }
    ...
    function editFile() {
        ...
        if (row) {
        	$.ajax({
                type: 'POST',
                url: Ams.ctxPath + '/jobSchedule/vehiclePollution/get',
                data: {'uuid': row.uuid},
                success: function (result) {
                 $('#file').filebox({
        			...
        			url = Ams.ctxPath + '/jobSchedule/vehiclePollution/saveFile';
                },
                dataType: 'json'
            });
        } else {
        ...
        if (row) {
            $.messager.confirm("提示信息", "确认删除当前选中的记录吗", function (r) {
                if (r) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: Ams.ctxPath + '/jobSchedule/vehiclePollution/delete',
