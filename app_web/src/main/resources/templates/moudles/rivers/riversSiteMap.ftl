...
<body>
	...
	<div class="container" >
				...
				<div class="map-tool-container" style=" z-index:1;">
				    <div class="map-panel" style="position:absolute;top:72px;right:30px;">
                        <div class="map-panel-header">
                            <span class="title">
                                <span class="icon iconcustom icon-zhedie3"></span>入海河流点位分析监控图
                            </span>
                        </div>
                                    ...
                                    <div class="theme-content">
                                        ...
                                        <div class="easyui-table-light">
                                            <table id="dg_enterprise" class="easyui-datagrid"
                                               ...
                                               url="${request.contextPath}/rivers/riversSiteInfo/getRiversSiteInfoList"
                                               data-options="
                                                rownumbers:false,
                                                singleSelect:true,
                                                striped:false,
                                                ...
                                                <tr>
                                                    ...
                                                    <th field="POINTNAME" width="180">名称</th>
                                                    <th field="REGIONNAME" width="120">行政区划名称</th>
		...
		/* 	//筛选点击 默认隐藏
			...
			function getLeftMenuColmnName(pointCode){
				$.ajax({
					type : 'POST',
					url : '${request.contextPath}/rivers/riversSiteMonitorData/getLeftMenuColumnName',
					data :{'pointCode' : pointCode},
					success : function(data) {
						var str = "";
						for( var i = 0 ; i< data.length ; i++){
							str = str +'<span pointCode="'+pointCode+'" id="'+data[i]["codePollute"]+'"    value="'+data[i]["polluteName"]+'">'+data[i]["polluteName"]+'</span>'
						}
			...
			function addMakerStation(stationArray) {
				for (var i = 0; i < stationArray.length; i++) {
					...
					if (markerMap.get(record.POINTCODE) == null) {
						imagePath = "${request.contextPath}/static/images/sea.png";
						iconMap = new fjzx.map.Icon(imagePath,new fjzx.map.Size(30, 40));
						myMarker = new fjzx.map.Marker(point, {
							icon : iconMap,
							map : map,
							title :record.POINTNAME
						});
					} else {
						myMarker = markerMap.get(record.POINTCODE);
					}
					idArray[i] = record.POINTCODE;
					myMarker.type = "river";
					myMarker.recordJson = record;
					map.addOverlay(myMarker);
					myMarkMap.put(record.POINTCODE, myMarker);
					addClickHandler(record, myMarker);
				    markerMap.put(record.POINTCODE, myMarker);
            	...
            	$.ajax({
					...
					success : function(data) {
						...
						$('#monitorDlg').dialog('open')
								.dialog('center')
								.dialog('setTitle','入海河流点位详情');
						getDeviceInfo(record);
						getLeftMenuColmnName(record.POINTCODE);
			...
			function getDeviceInfo(info) {
				...
				document.getElementById('deviceName').innerHTML = info.POINTNAME;
				document.getElementById('pointInfo').innerHTML = '<span>经度：'
						+ info.LONGITUDE + '</span>' + '<span>纬度：' + info.LATITUDE
						+ '</span>' + '<span>行政区划名称：' + info.REGIONNAME
						+ '</span><span>行政区划编码：' + info.codeRegion + ' </span>'
						+ '<span>流域名称：' + info.WSYSTEMNAME + ' </span>'+ '<span>流域编码：' + info.codeWsystem + '</span>';
			}
			...
			function getMonitorInfoByStationId(data) {
				...
				optionYarr = [];
				xAxisData = [];
				optionYarr = data["pollutantValueArr"];
				xAxisData = data["monitorTimeArr"];
		   ...
		   function addMenuLeftClick(){
				...
				for(var i = 0 ; i <pollutionCodeListSpan.length;i++){
					$(pollutionCodeListSpan[i]).click(function(){
						...
						menuLeftResolve($(this).attr("id"),$(this).attr("pointCode"),$(this).attr("value"));
		   ...
		   function defaultSelect(target){
				...
				menuLeftResolve(target.attr("id"),target.attr("pointCode"),target.attr("value"));
		   }
		   
			function menuLeftResolve(id,pointCode,titleName){
				  $.ajax({
					type : 'POST',
					url : '${request.contextPath}/rivers/riversSiteMonitorData/getRiversSiteMonitorDataList',
					data : {
						'pointCode' : pointCode,
						'columnCode' : id
					},
