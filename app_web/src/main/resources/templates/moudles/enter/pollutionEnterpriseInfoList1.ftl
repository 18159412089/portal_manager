...
<body>
	...
	<div class="easyui-layout" fit=true>
			...
			<div id="searchBar" class="searchBar">
				<form id="searchForm">
                  <div class="inline-block">
                   ...
                   <div class="inline-block">
                       <label  class="textbox-label textbox-label-before" title="标准污染源名称">标准污染源名称:</label>
                   ...
                   <div class="inline-block">
					...
					<a href="javascript:void(0)" class="easyui-linkbutton btn-orange" data-options="iconCls:'icon-redo'" onclick="exportData()">导出模板</a>
                    <a href="javascript:void(0)" class="easyui-linkbutton btn-primary" data-options="iconCls:'icon-add'" onclick="importExcel()">导入Excel</a>
				</form>
                ...
                <form method="post" enctype="multipart/form-data" id="resource">
                    ...
                    <a href="javascript:void(0)" id="fileUpload" onclick="subResource()"  class="easyui-linkbutton btn-primary">上传</a>
				</form>
				</div>
			</div>
			<!-- 
		...
		<table id="dg" class="easyui-datagrid" tyle="width: 100%; height: auto;" toolbar="#toolbar" 
			...
			<thead>
				<tr>
					<th field="entercode" width="10%">污染源编码</th>
					<th field="entername" width="10%">标准污染源名称</th>
					<th field="companyname" width="10%">企业名称</th>
					<th field="codeEntertype" width="10%">企业类型</th>
					<th field="codeRegistertype" width="10%">登记注册类型</th>
					<th field="trade" width="10%">所属行业</th>
					<th field="codeTrade" width="10%">行业代码</th>
					<th field="enteraddress" width="10%">企业地址</th>
					<th field="wsystem" width="10%">所属流域</th>
...
<div id="dlg-detail" class="easyui-dialog" style="width: 420px"
	...
	<script>
		...
		function exportData() {
			doSearch();
			window.open(Ams.ctxPath + '/enter/pollutionEnterpriseInfo/getListExportTemplate');
		}
        ...
        function importExcel() {
            alert('提示：请先下载模板，填写后再进行导入功能！');
            $('#dlg3').dialog('open').dialog('center').dialog('setTitle', '导入Excel');
            $('#fm').form('clear');
            $('#code').textbox({readonly: false});
        }
        ...
        function subResource(){
            ...
            if (files != '') {
                ...
                $.ajax({
                    url: "${request.contextPath}/enter/pollutionEnterpriseInfo/import",
                    type: 'POST',
                    data: formData,
                    async: false,
                    cache: false,
                    ...
                    success: function (returnInfo) {
                        if(returnInfo.type == "S"){
                            alert('上传成功！');
                            window.location.href="${request.contextPath}/enter/pollutionEnterpriseInfo/index";
						}else{
                            alert('上传失败！');
						}
                    },
