            ...
            <div class="column-panel">
                <div class="column-panel-header">
                    ...
                    <span style="position: absolute;right: 42%;padding: 8px;padding-top: 0px;">
						<font style="font-size: 180%;font-weight: bold !important;" id="title">环保督察整改汇总图表</font>
					</span>
                    <a id="printImg" onclick="Ams.doPrint('myPrint')" class="printing-button">
                        <span class="icon iconcustom yl_print" /></span> 打印
                    </a>
                    <a id="exportImg" onclick="Ams.exportPdfById('myPrint','漳州市环保督察整改汇总情况')" class="printing-button">
...
<script type="text/javascript">
    ...
    function rowTable(data) {
        ...
        newRow += '<span class="control-content"><a href="javascript:void(0)" class="easyui-linkbutton" onClick="show(\''+dataVal.dutyDepartment+'\')">'+(Ams.isNoEmpty(dataVal.dutyDepartment)==false?"":dataVal.dutyDepartment) +'</a></span>';
        newRow += '</div>';
        newRow += '<div class="data-group layui-col-xs4">';
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data.result,function(i){
                    pointName.push(data.result[i].pointName+','+data.result[i].pointCode);
                    var str = {
                        value:data.result[i].count,
                        tooltip:
                            {
                                formatter : data.result[i].pointCode
                            }
                    };
                    projectCount.push(str);
                });
                $('#title').text((Ams.isNoEmpty(data.qx)==true?data.qx:'漳州市')+'环保督察整改汇总图表')
