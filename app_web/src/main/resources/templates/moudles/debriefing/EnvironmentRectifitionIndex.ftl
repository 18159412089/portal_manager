            ...
            <div class="column-panel">
                <div class="column-panel-header">
                    ...
                    <span style="position: absolute;right: 42%;padding: 8px;padding-top: 0px;">
						<font style="font-size: 180%;font-weight: bold !important;" id="title">环保督察总体情况</font>
					</span>
                    <a id="printImg" onclick="Ams.doPrint('myPrint')" class="printing-button">
                        <span class="icon iconcustom yl_print" /></span> 打印
                    </a>
                    <a id="exportImg" onclick="Ams.exportPdfById('myPrint','漳州市环保督察总体情况')" class="printing-button">
                ...
                <div class="column-panel-body" style="margin-top:28px; ">
                    ...
                    <div id="exportPic" style="overflow: hidden" >
                        ...
                        <div class="ztqk-right-box" >
                            ...
                            <div id="information"  class="information-text" >总体情况 我市中央环保督察问题24个，截至3月25日，我市应完成整改任务13个，11个达到序时进度。已完成整改13个，完成市级验收13个，完成行业审查4个。
                            </div>
                        </div>
...
<script type="text/javascript">
    ...
    function getDecription(id,name,status){
        ...
        $('#dgDescription').datagrid({
            url:"${request.contextPath}/environment/rectifition/getDecription?id="+id+"&status="+status+"&startTime="+startTime+"&endTime="+startTime,
            onLoadSuccess:function(data){
//            	var data = $('#dgDescription').datagrid('getData');
    ...
    function getDecriptionAll(name,value){
        ...
        $('#dgDescriptionAll').datagrid({
            url:"${request.contextPath}/environment/rectifition/getDecriptionAll?status="+name+"&startTime="+startTime+"&endTime="+startTime,
        });
    }
    ...
    function showDescribe(id){
        ...
        $('#dgDetail').datagrid({
            url:"${request.contextPath}/environment/rectifition/getDetail?id="+id,
        });
    ...
    function getdata(startTime,endTime){
        $.ajax({
            type: 'POST',
            url: Ams.ctxPath + '/environment/rectifition/getEcharts',
            async:true,
            data : {
                startTime:startTime,
                endTime:endTime
            ...
            success: function (data) {
                var yearnum,title;
                if (Number(endTime.split("-")[0]) <= 2000) {
                    yearnum = Ams.dateFormat(new Date(), "yyyy年MM月dd日");
                    title = '华东督查问题';
                } else {
                    yearnum = endTime.split("-")[0]+" 年";
                    title = '中央环保督察问题';
                }
                $('#title').text((Ams.isNoEmpty(data.qx) == true ? data.qx : '漳州市') + '环保督察总体情况');
                var remark = "截至 "+yearnum+","+(Ams.isNoEmpty(data.qx)==true?data.qx:'漳州市')+title+" ，"
                    + "尚未启动任务 "+data.ns+" 个，"
                    + "未达到序时进度任务 "+data.nr+" 个，"
                    + "达到序时进度任务 "+data.ot+" 个，"
                    + "超过序时进度任务 "+data.ps+" 个，"
    ...
    function getAirQualityStatistics(startTime,endTime){
        $.ajax({
            type: "post",
            url:  Ams.ctxPath + "/environment/rectifition/getPieEcharts",
            async:true,
            data : {
                startTime:startTime,
                endTime:endTime
