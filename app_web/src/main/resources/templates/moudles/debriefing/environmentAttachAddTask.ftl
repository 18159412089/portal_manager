...
<div class="easyui-layout" fit=true style="margin-top:70px;width: 100%">
	<div class="data-display-container">
	    <div class="title">
	        <span>漳州市八闽快讯(一本账)汇总表</span>
	    </div>
    </div>
    <div id="toolbar">
        <div style="padding:10px;" id="searchBar">
            <a href="javascript:void(0)" class="easyui-linkbutton btn-primary" onclick="jump()">任务类表</a>
        </div>
    </div>
    	...
    	<table cellpadding="5" >
    		...
    		<tr>
    			<td></td>
    			<td>
    				<a href="javascript:void(0)" class="easyui-linkbutton btn-primary" onclick="submit()">提交</a>
    			</td>
    		</tr>
    	</table>
    </form>
    ...
    <form id="fm" method="post" novalidate style="margin:0;padding: 10px !important;">
    	<table>
    		...
    		<tr>
    			<td></td>
    			<td>
    				<a href="javascript:void(0)" class="easyui-linkbutton btn-primary" onclick="addRelation()">提交</a>
...
<script>
	...
	function opera(val, row, index) {
	    return '<div id="rowid_'+index+'">'+Ams.setImageEdit()+'<a href="javascript:void(0)" class="easyui-linkbutton" onclick="edit('+index+')">编辑</a>&emsp;' +
				''+Ams.setImageDelete()+'<a href="javascript:void(0)" class="easyui-linkbutton" onclick="del('+index+')">删除</a></div>';
	}
	...
	function edit(index){
        ...
        if(row){
        	...
        	$("#"+div).html('<a href="javascript:void(0)" class="easyui-linkbutton" onclick="sure('+index+')">确定</a>');
        	//
            //获取编辑行
	...
	function sure(index){
        ...
        if(row){
        	...
        	var div = "rowid_"+index;
        	$("#"+div).html(Ams.setImageEdit()+'<a href="javascript:void(0)" class="easyui-linkbutton" onclick="edit('+index+')">编辑</a>&emsp;'+Ams.setImageDelete()+'<a href="javascript:void(0)" class="easyui-linkbutton" onclick="del('+index+')">删除</a>');
       		$.ajax({
                   type: 'POST',
