...
function getWpfsqy() {
	if ($("#wpfsqy").hasClass('choiced')) {
		...
		$('#wpfsqyDg').datagrid({url:'/zphb/enviromonit/water/nationalSurfaceWater/getWpfsqyList'});
		...
		$.ajax({
			type : "post",
			url : "/zphb/enviromonit/water/nationalSurfaceWater/getPolluteWaterData",
			async : true,
			success : function(result) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					markerMap.put(data[i].qymc, tempMarker);
				}
			},
			error : function() {
			},
...
function getWpfsqyInfo(info){
	...
	$('#wastewaterCompanyFileInfoSet').attr("href", "/zphb/enviromonit/water/wastewaterFileAttachment?pid=" + $("#pid").val() + "&companyName=" + info.qymc);
	$('#wastewaterCompanyInfoSet').attr("href", "/zphb/enviromonit/water/wastewaterCompanySet?pid=" + $("#pid").val() + "&qymc=" + info.qymc);
	$.ajax({
		type : 'POST',
		url : '/zphb/enviromonit/water/nationalSurfaceWater/getPolluteWaterDataDetail',
		async : true,
		data : {qymc : info.qymc},
		success : function(result) {
			var data = result.data;
