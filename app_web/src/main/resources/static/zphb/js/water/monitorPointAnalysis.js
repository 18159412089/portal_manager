...
function getMnPointInfo(info) {
    ...
    $.ajax({
        type : "post",
        url : '/zphb/enviromonit/water/nationalSurfaceWater/getPolluteList',
        async : true,
        data : {
            pointCode : info.mn,
...
function searchWaterPointBar() {
    ...
    $.ajax({
        type : "post",
        url : '/zphb/enviromonit/water/nationalSurfaceWater/getTrendChart',
        async : true,
        data : {
            mn : pointCodes,
...
function getFactorCount() {
    $.ajax({
        type : "post",
        url : '/zphb/enviromonit/water/nationalSurfaceWater/getFactorCount',
        async : true,
        data : {
...
function addMoreData() {
    ...
    var pid = $('#pid').val();
    if (category == "1") {
        $("#moreData a").attr("href", "/zphb/enviromonit/water/nationalSurfaceWater/index?pid="+pid+"&pointCode=" + pointCode);
    } else if (category == "2") {
        $("#moreData a").attr("href", "/zphb/enviromonit/water/wtHourData/index?pid="+pid+"&pointCode=" + pointCode);
    } else if (category == "3") {
        $("#moreData a").attr("href", "/zphb/enviromonit/water/wtCityHourData/index?pid="+pid+"&pointCode=" + pointCode);
    }
    $("#attach a").attr("href", "/zphb/environment/waterAttachment/index?pid="+pid+"&pointCode=" + pointCode);
...
function getWaterMonitorAttachement(pcode) {
    ...
    $('#attachmentIdDg').datagrid({
        url: '/zphb/environment/waterAttachment/list',
        queryParams: {
            pointCode:pcode,
            isShow:"1",
            enable:"1",
...
function operation(value,row){
        if (row.type == 3) {
            return '<div class="link-box"><a class=\'look-tag\' href="javascript:onClick= play(\''+row.mongoid+'\')" target="_blank" ><span class="icon iconcustom look-icon"></span>查看</a><a href="/zphb/environment/waterAttachment/downloadFile/'+row.mongoid+'/'+row.type+'"><span class="icon iconcustom"></span>下载</a></div>';
        } else {
            return '<div class="link-box"><a  class=\'look-tag\' href="/zphb/environment/waterAttachment/download/'+row.mongoid+'/'+row.type+'" target="_blank"><span class="icon iconcustom look-icon"></span>查看</a><a href="/zphb/environment/waterAttachment/downloadFile/'+row.mongoid+'/'+row.type+'"><span class="icon iconcustom"></span>下载</a></div>';
        }
}
...
function findAttachmentByName(){
       if ($("#attachementName").val()==undefined ||$("#attachementName").val()==null||$("#attachementName").val()==''){
        ...
        $('#attachmentIdDg').datagrid({
            url: '/zphb/environment/waterAttachment/findAttachementByName',
            queryParams: {
                pointCode:pointCode,
