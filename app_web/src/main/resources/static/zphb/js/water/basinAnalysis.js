...
function getBasin() {
	if ($("#basin").hasClass('choiced')) {
		...
		$.ajax({
			type : "post",
			url : "/zphb/enviromonit/water/nationalSurfaceWater/getBasinMonitor",
			async : true,
			success : function(result) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					var clickDiv = null ;
		            clickDiv = "<div style='width: 120px;text-align: left;'><div class='tdt-infowindow-content' style='width: 40px;margin-left:-10px;'>"
						...
						+"</div><p style='font-weight:bold;margin-left:-10px'>"+data[i].name+"</p></div>";
		            //创建pint
					//创建地图marker点
		            var  myMarker = new fjzx.map.Marker( new fjzx.map.Point(data[i].lng, data[i].lat), {
		        	    markerHtml : clickDiv,
						map : map,
						isShowIcon :false,
						title:data[i].name,
					...
					markerMap.put(data[i].code, myMarker);
...
function getBasinInfo(info) {
	...
	var pid = $('#pid').val();
    $("#fjlbid").attr("href",encodeURI("/zphb/environment/waterAttachment/river?pid="+pid+"&pointCode="+info.code+"&pointName="+info.name));//附件列表
...
//			+ '<span>目标水质：' + target + '</span><span style="color:'+resultColor+'">综合水质评价：' + resultQuality + '</span>';
	...
	$.ajax({
		type : "post",
		url : "/zphb/enviromonit/water/nationalSurfaceWater/getOtherBasinMonitorInfo",
...
function searchAnalysis(){
	...
	if(basinName != undefined || basinName != null || basinName==""){
		$.ajax({
			type: "post",
			url:  Ams.ctxPath + "/zphb/enviromonit/water/nationalSurfaceWater/getBasinYearAnalysis",
			data : {
				name : basinName,
				thisYear:thisYear, 
				lastYear:lastYear,
...
function searchAnalysis_tb(){
	...
	if(basinName != undefined || basinName != null || basinName==""){
		$.ajax({
			type: "post",
			url:  Ams.ctxPath + "/zphb/enviromonit/water/nationalSurfaceWater/getBasinYearAnalysis_tb",
			data : {
				name : basinName,
				polluteCode:polluteCode
			},
...
function searchAnalysis_njdb(){
	...
	if(basinName != undefined || basinName != null || basinName==""){
		$.ajax({
			type: "post",
			url:  Ams.ctxPath + "/zphb/enviromonit/water/nationalSurfaceWater/getBasinYearAnalysis_njdb",
			data : {
				name : basinName,
				thisYear:thisYear, 
				lastYear:lastYear,
...
function seeAttach(mongoid,type) {
    if (type == 3) {
        ...
        return "<div>"+Ams.setImageSee()+"<a href='/zphb/environment/waterAttachment/download/" + mongoid + "/" + type + "' class='easyui-linkbutton' target='_blank'>查看</a></div>";
    }
}
