...
function CreateFormPage(strPrintName, printDatagrid,title) {
        ...
        $(columns).each(function (index) {
            ...
            for (var i = 0; i < columns[index].length; ++i) {
                if (!columns[index][i].hidden) {
                    if (columns[index][i].width != undefined && columns[index][i].title != '操作') {//操作栏不要
        ...
        $(nl).each(function (j) {
            ...
            if (e + 2 == nl[j].f.length) {
                ...
                if(rows[i][nl[j].f]==undefined){
                    ...
                    if (nl[j].f.indexOf('Date')>-1){//字段名称
                        tableString += Ams.timeDateFormat(rows[i][nl[j].f]);
                    }else
                    {
    ...
    window.open("/static/print/printTest.html", strPrintName,
