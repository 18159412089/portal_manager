...
function initFunction() {//初始化函数一个就够没必要多个
...
var pointNameForArea

function getWaterAndCityNum() {
    //获取水、气各个标签的各区县数值Json
    $.ajax({
        type: 'POST',
        ...
        success: function (result) {
            ...
            for (var i = 0; i < result.county.length; i++) {
                html += '<li class="item"><div class="personnel-parent alone-personnel-parent tableClass" ' +
                    'onclick="areaClick(this,\'' + result.county[i] + '\',\'' + result.abridge[i] + '\',' +
                    '\'' + result.jd[i] + '\',\'' + result.wd[i] + '\')">' + result.county[i] +
                    '<span id="' + result.abridge[i] + '">0</span>个</div></li>';
                areJWDPoint[areaCountyNamelist[result.abridge[i]]]={"jd":result.jd[i],"wd":result.wd[i]};
            }
            $('#areaList').html(html);
            pointNameForArea = result.data;

            loadAreaNum(result.data, result.county);
        }
    });
}
...
function words() {
    if ($('.contaminated-personnel-ul').find('li').length == 6) {
        var html = '  截止<i>' + Ams.dateFormat(new Date(), "yyyy年MM月dd日")
            + '</i>各县（市、区）共上报涉环保5大类型污染源<i>' + $('.no-choice').length + '</i>种。其中：芗城区<i>' + countWordAnalysis.xcq + '</i>个、'
            + '龙文区<i>' + countWordAnalysis.lwq + '</i>个、龙海区<i>' + countWordAnalysis.lhs + '</i>个、漳浦县<i>'
            + countWordAnalysis.zpx + '</i>个、云霄县<i>' + countWordAnalysis.yxx + '</i>个、诏安县<i>' + countWordAnalysis.zax + '</i>个、'
            + '东山县<i>' + countWordAnalysis.dsx + '</i>个、平和县<i>' + countWordAnalysis.phx + '</i>个、南靖县<i>'
            + countWordAnalysis.njx + '</i>个、长泰县<i>' + countWordAnalysis.ctx
            + '</i>个、华安县<i>' + countWordAnalysis.hax + '</i>个、'
            + ' 漳州开发区<i>' + countWordAnalysis.zsjzzkfq + '</i>个、古雷开发区<i>' + countWordAnalysis.glgjjkfq + '</i>个、台商投资区<i>'
            + countWordAnalysis.tstzq + '</i>个、高新区<i>' + countWordAnalysis.zzgxjscykfq +
            '</i>个、常山开发区<i>' + countWordAnalysis.cshqjjkfq + '</i>个。';
        $('#analysis').html(html);
    } else {
        var html = '  截止<i>' + Ams.dateFormat(new Date(), "yyyy年MM月dd日") + '</i>各县（市、区）上报涉环保'
            + $('.contaminated-personnel-ul').find('li').eq(1).find('.personnel-parent').find('span').text()
            + '<i>' + $('.no-choice').length + '</i>种。其中：芗城区<i>' + countWordAnalysis.xcq + '</i>个、'
            + '龙文区<i>' + countWordAnalysis.lwq + '</i>个、龙海区<i>' + countWordAnalysis.lhs + '</i>个、漳浦县<i>' + countWordAnalysis.zpx
            + '</i>个、云霄县<i>' + countWordAnalysis.yxx + '</i>个、诏安县<i>' + countWordAnalysis.zax + '</i>个、'
            + '东山县<i>' + countWordAnalysis.dsx + '</i>个、平和县<i>' + countWordAnalysis.phx + '</i>个、南靖县<i>' + countWordAnalysis.njx
            + '</i>个、长泰县<i>' + countWordAnalysis.ctx + '</i>个、华安县<i>' + countWordAnalysis.hax + '</i>个、'
            + '  漳州开发区<i>' + countWordAnalysis.zsjzzkfq + '</i>个、古雷开发区<i>' + countWordAnalysis.glgjjkfq + '</i>个、台商投资区<i>'
            + countWordAnalysis.tstzq + '</i>个、高新区<i>' + countWordAnalysis.zzgxjscykfq + '</i>个、常山开发区<i>'
            + isNum(countWordAnalysis.cshqjjkfq) + '</i>个。';
        $('#analysis').html(html);
    }
}
...
$('.no-choice').click(function () {
    ...
    if (cl_n.hasClass('choiced')) {
        ...
        clearPointMapByNochoice(tid)//清除点位信息
        loadAreaNumForClick(tid, false)
    } else {
...
function pointAboutPollution(tid) {
    //关于数据的描点情况书写点位信息
    if (!Ams.isNoEmpty(pointData[tid]))
        return;
    var data = pointData[tid];
    var targetIcon = newIconMap[tid];
    var lng = null;
    var lat = null;
    var name = null;
    ...
    for (i = 0; i < data.length; i++) {
        ...
        arraylist.push(tempMarker);
        addCaseClickEvent(tempMarker, data[i]);
    }
    wrylxPointMap[tid] = arraylist;
}
    ...
    drawGeojson.setSelectAction(function (e, selectInteraction) {
        ...
        features.forEach(function (feature, index, array) {
            ...
            if (feature.id_ != "hasSelectedFeature") {
                ...
                map.setCenterAndZoom(new fjzx.map.Point( areJWDPoint[areaName].jd, areJWDPoint[areaName].wd),11)
                colorForTab(areaName)
            }
...
function areaPointMapData() {
    ...
    $('.no-choice').each(function () {//获取选中的污染源点击事件
        if ($(this).hasClass("choiced")) {
            var tid = $(this).attr("id");
            var dataPoint = pointData[tid];//获取点位信息
            ...
            for (var i = 0; i < length; i++) {
                ...
                if (checkPointInArea(lng, lat, areaName)) {
                    ...
                    map.addOverlay(tempMarker);
                    arrayList.push(tempMarker)
                    addCaseClickEvent(tempMarker, dataPoint[i])
                }
            }
            wrylxPointMap[tid] = arrayList;
        }
...
function areaClick(idthis, areaN, idVal, jd, wd) {
    if (($(idthis).hasClass("tableClass collapsed") || $(idthis).hasClass("select-part collapsed")) && areaClicChange == 0) {
        ...
        map.setCenterAndZoom(new fjzx.map.Point("117.76264335320418", "24.369671575212295"),9.45)
        $(".tableClass").css({"display":"block"})
        return;
    }
    if(jd!=null&&wd!=null){

        map.setCenterAndZoom(new fjzx.map.Point(jd, wd),11)
    }
    setCountyArea(areaN);
    areaClicChange = 0;
    areaName = areaN;
    ...
    getTotalForTaglib1();//计算统计数据点击地图区域之后;
    html += '</ul></div>';
...
function getLabel(tid, areaName, areaId, array) {
    var result2 = '';
    var total = 0;
    if (Ams.isNoEmpty(array[wrylxMc[tid]])) {
        var url = "/env/pollution2/getEnterpriseByQxAndLxAndZl?zl=" + wrylxMc[tid] + "&qx=" + areaName + "&lx=" + wrylxParamType[tid];
        if (array[wrylxMc[tid]] != 0) {
            result2 += '<li class="item"><div class="personnel-parent alone-personnel-parent" ' +
                'onclick="dataRelaod(\'' + tid + '\',\'' + tid + areaId + '\', \'' + url + '\')">' + wrylxMc[tid] + ' <span>' + array[wrylxMc[tid]] + '个</span></div>' +
            ...
            total = array[wrylxMc[tid]];
        }
    }
    setPollutionData1(tid, total);
    return result2;
}
...
function dataRelaod(tid, dgid, url) {
    ...
    $('#' + dgid).datagrid({
        ...
        onClickRow: function (index, data) {
            ...
            clearAllMapPoint(data.jd, data.wd, data.mc, newIconMap[tid], tid, data);
...
function doSearch() {
    $('#searchDG').datagrid({
        ...
        onClickRow: function (index, data) {
            map.setCenter(new fjzx.map.Point(data.jd, data.wd));
            for (var obj in wrylxMc) {
                if (wrylxMc[obj] == data.wryzl) {
                    clearAllMapPoint(data.jd, data.wd, data.mc, newIconMap[obj], obj, data);
                    break;
                }
            }
        },
...
var countWordAnalysis = {};//插值分析数值
function loadAreaNum(data, countyList) {
     for (var countyName in areaCountyNamelist) {
        var countNum = 0;
        for (var obj in iconImg) {//iconImg只是为了获取obj就是类型的id
            for (var i = 0; i < data[pollutionMapNewPollution[obj]].length; i++) {
                countNum += isNum(data[pollutionMapNewPollution[obj]][i][areaCountyNamelist[countyName]]);
            }
        }
        countWordAnalysis[countyName] = countNum;
       // $("#" + countyName).html(countNum);
    }
    words();//插值分析的数据;
}

function loadAreaNumForClick(tid, flag) {
    //点击no-choice后数据的改变如果点击为选中就增加如果为非选中状态就相减
    for (var countyName in areaCountyNamelist) {
        var countNum = Number($("#" + countyName).text());
        for (var i = 0; i < pointNameForArea[pollutionMapNewPollution[tid]].length; i++) {
            flag ? countNum += isNum(pointNameForArea[pollutionMapNewPollution[tid]][i][areaCountyNamelist[countyName]]) :
                countNum -= isNum(pointNameForArea[pollutionMapNewPollution[tid]][i][areaCountyNamelist[countyName]]);
        }
        $("#" + countyName).html(countNum);
    }
}
...
//污水处理厂对应涉水工业企业;常规排口对应海洋排污口;水库对应海洋固体废物;工业废气企业对应vocs企业;
// 微型水质自动站对应高架源企业;小流域对应散乱污企业;废气排口->工业固废;工地->工业危险废物;工业废气企业->生态污染源企业;

var pollutionMapData;

function pollutionDataPoint() {
    if (Ams.isNoEmpty(pollutionMapData)) {
        initPollutionMap(pollutionMapData);
        return;
    }
    $.ajax({
        ...
        success: function (result) {
            ...
            initPollutionMap(result);//都是加载一次就可以了
        },
        error: function () {
        },
        dataType: 'json'
...
var countHY = 0;
var pointData = {};//污染源点数据

var wrylxMcNum = {};//污染源类型非点击地图点击事件数量
var wrylxMcNumForArea = {};//污染源类型地图点击事件数量
function wryzlPollutionNum() {//污染源种类数字统计计算
    if (countPollutionNum > 0) {//如果数据已经算完了name就不需要再重新计算一遍
        getTotalForTaglib();
        return;
    }
    countPollutionNum = isNum(wrylxMcNum.wsclc);
    countHY = isNum(wrylxMcNum.cgpk);
    countHY += isNum(wrylxMcNum.sk);
    countPollutionAIRNum = isNum(wrylxMcNum.gyfsqy);
    countPollutionAIRNum += isNum(wrylxMcNum.wxszzdz);
    countPollutionAIRNum += isNum(wrylxMcNum.xly);
    countPollutionAIRNum += isNum(wrylxMcNum.fdlydy);
    countTR = isNum(wrylxMcNum.fqpk);
    countTR += isNum(wrylxMcNum.gd);
    countTR += isNum(wrylxMcNum.sghfc);
    countST = isNum(wrylxMcNum.sbc);
    countST += isNum(wrylxMcNum.gyfqqy);
    getTotalForTaglib();
}

function setPollutionData(tid) {
    var icon = iconImg[tid];//

    $("#" + tid).empty();
    $("#" + tid).append('<span><img src="' + icon + '"></span>' + wrylxMc[tid] + '(' + wrylxMcNum[tid] + ')');
}
...
function getTotalForTaglib1() {//每次都需要重新计算
    countPollutionNum1 = isNum(wrylxMcNumForArea.wsclc);
    countHY1 = isNum(wrylxMcNumForArea.cgpk);
    countHY1 += isNum(wrylxMcNumForArea.sk);
    countPollutionAIRNum1 = isNum(wrylxMcNumForArea.gyfsqy);
    countPollutionAIRNum1 += isNum(wrylxMcNumForArea.wxszzdz);
    countPollutionAIRNum1 += isNum(wrylxMcNumForArea.xly);
    countPollutionAIRNum1 += isNum(wrylxMcNumForArea.fdlydy);
    countTR1 = isNum(wrylxMcNumForArea.fqpk);
    countTR1 += isNum(wrylxMcNumForArea.gd);
    countTR1 += isNum(wrylxMcNumForArea.sghfc);
    countST1 = isNum(wrylxMcNumForArea.sbc);
    countST1 += isNum(wrylxMcNumForArea.gyfqqy);
...
function isNum(isNumber) {//是否是数字不是的话返回0
    if (Ams.isNoEmpty(isNumber)) {
        return isNumber;
    }
    return 0;
}

function getNewIconMap(iconImgName) {//获取地图中的icon;
    return new fjzx.map.Icon(
        iconImgName,
        {
...
function setPollutionData1(tid, total) {
    var icon = iconImg[tid];
    var icon = iconImg[tid];
    $("#" + tid).empty();
    $("#" + tid).append('<span><img src="' + icon + '"></span>' + wrylxMc[tid] + '(' + total + ')');
    wrylxMcNumForArea[tid] = total;

}


function initPollutionMap(result) {
    //初始化地图设置数值并且获取到完整的类型不多不少比
    // 如(ollutionInfo2有两个类型pollutionInfo3有三个类型然后数据就是这样组装完成;
    //初始化地图要完成的是由1布点2将布点的数写成静态的数据将数据进行大小的补充
    //clearAllPointMap();
    $('.no-choice').each(function () {
        objId = $(this).attr("id")
        pointData[objId] = result[pollutionMapNewPollution[objId]];
        wrylxMcNum[objId] = result[pollutionMapNewPollution[objId]].length;
        if($(this).hasClass("choiced")){
            pointAboutPollution(objId);
        }
        setPollutionData(objId)
    })
    wryzlPollutionNum();
}
...
//全部用一个数组就可以了就是说虽然清除了但是不需要再重新加载只需要再次使用就可以了;
function clearAllPointMap() {
    for (var obj in wrylxPointMap) {
        for (var i = 0; i < wrylxPointMap[obj].length; i++) {
            map.removeOverlay(wrylxPointMap[obj][i]);
        }
    }
    clearOnePoint()
}
...
//点击布点问题布点;
function pushPointMapByChoice(tid) {
    pointAboutPollution(tid);//点击添加数据

}

function clearOnePoint() {//清除一个点位;
    if (oneMapPoint.length == 1) {
        map.removeOverlay(oneMapPoint[0]);
    }
}

//清除布点问题点击污染源种类进行清除只有点击的时候才可以出发
function clearPointMapByNochoice(tid) {
    for (var i = 0; i < pointData[tid].length; i++) {
        map.removeOverlay(wrylxPointMap[tid][i]);
    }
    clearOnePoint();
}

var newIconMap = {
    "wsclc": getNewIconMap("/static/images/ssgyqy-icon.png"),
    "cgpk": getNewIconMap("/static/images/hypk.png"),
    "sk": getNewIconMap("/static/images/hygygtfw-icon.png"),
    "gyfsqy": getNewIconMap("/static/images/VOCs-icon.png"),
    "wxszzdz": getNewIconMap("/static/images/gjyqy-icon.png"),
    "xly": getNewIconMap("/static/images/slwqy-icon.png"),
    "fqpk": getNewIconMap("/static/images/gyfq-icon.png"),
    "gd": getNewIconMap("/static/images/gywxfw-icon.png"),
    "gyfqqy": getNewIconMap("/static/images/stwry-icon.png"),
    "sbc": getNewIconMap("/static/images/ybgy-icon.png"),
    "fdlydy": getNewIconMap("/static/images/fdlydy-icon.png"),
    "sghfc": getNewIconMap("/static/images/sghfc-icon.png")
};//地图中显示的图标
var wrylxParamType = {//污染源传入后端的参数大类型的数据就是在污染源中的数据的类型有这个数组进行解决
    "wsclc": "水",
    "cgpk": "海洋",
    "sk": "海洋",
    "gyfsqy": "大气",
...
var iconImg = {
    "wsclc": "/static/images/ssgyqy-icon.png",
    "cgpk": "/static/images/hypk.png",
    "sk": "/static/images/hygygtfw-icon.png",
    "gyfsqy": "/static/images/VOCs-icon.png",
    "wxszzdz": "/static/images/gjyqy-icon.png",
    "xly": "/static/images/slwqy-icon.png",
    "fqpk": "/static/images/gyfq-icon.png",
    "gd": "/static/images/gywxfw-icon.png",
    "gyfqqy": "/static/images/stwry-icon.png",
    "sbc": "/static/images/ybgy-icon.png",
    "fdlydy": "/static/images/fdlydy-icon.png",
    "sghfc": "/static/images/sghfc-icon.png"
};//污染原类型中的右侧图标
/**
 * 添加点位标识数组
 * @param tid 点位标签id
 * @param tempMark
 */
var wrylxPointMap = {};//地图显示的点位tempmark;每次布点时添加
var areaCountyNamelist = {
    "ctx": "长泰县",
    "lhs": "龙海市",
    "phx": "平和县",
    "hax": "华安县",
    ...
    "zsjzzkfq": "招商局漳州开发区",
    "cshqjjkfq": "常山华侨经济开发区",
    "zzgxjscykfq": "漳州高新技术产业开发区",
    "glgjjkfq": "古雷港经济开发区",
};//县区列表拼音和县区对应这个是根据id来进行数据匹配的;
var wrylxMc = {
    "wsclc": "涉水工业企业",
    "cgpk": "海洋排污口",
    "sk": "涉海工业固废",
