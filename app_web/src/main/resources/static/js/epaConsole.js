...
var options = opt_options || {};
  ...
  //btn-group单选切换
    $("body").on("click.btngroup",".btn-group .btn",function(){
...
function getMapDate() {
	JointServiceBaseMap.getConsoleData(
			...
			function(data){
				...
				loadMonitorRegionList(data.monitorRegionList);
				//绘制在线人员标注点
				...
				//alert("标记物总数:"+map.getOverlays().length);
			},
			function(){
...
function loadUserList(dataList) {
	...
	}else { //只显示departmentShow 保存的部门人员
		for (var i = 0; i < dataList.length; i++) {
			var record =  dataList[i];
...
function showMakerLocation(record) {
	...
	if(record.latitude==null ||record.latitude==""||record.longitude==null ||record.longitude==""){
		 $.messager.show({
				title: '错误',
				msg: "企业坐标信息出错！"
			});
	...
	if(marker!=null){
		...
		if(marker.type == "POLLUTION"){
			openPollutionInfo(record,marker);
		}else if (marker.type =="MONITOR"){
			openInfo(record,marker);
		}else if (marker.type =="USER"){
			openPersonInfo(record,marker);
		}else if (marker.type =="MONITOR_BASIN"){
			openInfoBasin(record,marker);
		}else if(marker.type == "HYDRO"){
			openInfo(record,marker,true);
		}else{
			openInfo(record,marker,true);
		}
	}else{
...
//显示任务列表
function loadMonitorRegionList(dataList) {
	for (var i = 0; i < dataList.length; i++) {
		var record =  dataList[i];
		showPoint("MONITOR_REGION", record.id, record.name,record.longitude, record.latitude,record);
	}
}
...
function showPoint(type, recordId, recordName, x, y,record,showPointCallBack) {
	...
	}else if (type == "MONITOR_REGION") {//场所摄像头
		showMonitorRegionPoint(type, recordId, recordName, x, y,record,showPointCallBack);
	}else if (type == "TASK") {//事件
		showTaskPoint(type, recordId, recordName, x, y,record,showPointCallBack);
	}else if (type == "EVENT") {//事件
		showTaskPoint(type, recordId, recordName, x, y,record,showPointCallBack);
	}else if (type == "REPORT") {//问题受理
		showReportPoint(type, recordId, recordName, x, y,record,showPointCallBack);
	}else if (type == "CASE") {//投诉案件
		showCasePoint(type, recordId, recordName, x, y,record,showPointCallBack);
...
var recordtime = null;
function showMonitorRegionPoint(type, recordId, recordName, x, y,record,showPointCallBack){
	var imgPath = "images/marker_port_01.png";
	var clickDiv  = null;

	var leixing = '"MONITOR_REGION"';
	var content =" ";
	//var content = "<div>"+recordName+"<input type='hidden' name='maker_type' value='"+type+"' /><input type='hidden' name='maker_value' value='"+JSON.stringify(record)+"' /></div>";
	var title = record.name;
	var iconMap  = new fjzx.map.Icon("images/marker_port_01.png",  new fjzx.map.Size(24,26));
...
function showMarker(type,recordId,recordName,x,y,iconMap,clickDiv,content,title,showPointCallBack){
	...
	marker.addClick(function(e){
		var clicktype ="1";
		markerCilckEvent(e,clickDiv,marker,type,recordId,recordName);
	});
...
//地图点击覆盖物事件
function markerCilckEvent(e,clickDiv,marker,type,recordId,recordName){
	if(clickDiv != null){		
		var infoWindow = new fjzx.map.InfoWindow({infoWindow: clickDiv});//创建信息窗口对象
		marker.openInfoWindow(infoWindow);//打开信息窗口  
