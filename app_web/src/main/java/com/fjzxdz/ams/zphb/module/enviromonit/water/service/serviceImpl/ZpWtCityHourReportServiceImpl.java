package com.fjzxdz.ams.zphb.module.enviromonit.water.service.serviceImpl;
...
public class ZpWtCityHourReportServiceImpl implements ZpWtCityHourReportService {
    ...
    public JSONArray countMonitorSituation() {
        ...
        sql.append("  AND E.LAT_VALUE IS NOT NULL AND E.PE_TYPE = 'peType1' and E.qx like '%漳浦%'");
        //==常规口==
        ...
        //===污普废水企业==
        sql.append(" UNION ALL ");
        sql.append(" SELECT COUNT(DISTINCT QYMC) FROM POLLUTION_WATER_DATA where qx like '%漳浦%' ");
        //==小流域==
        sql.append(" UNION ALL ");
        sql.append(" SELECT COUNT(1) FROM WT_BASIN_MONITOR A where ischeck=1 and county like '%漳浦%'");
