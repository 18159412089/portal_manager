...
public class EnvironmentRectifitionServiceImpl implements EnvironmentRectifitionService {

    @SuppressWarnings("unused")
    private static Logger logger = LogManager.getLogger(EnvironmentRectifitionServiceImpl.class);
     ...
     * @param page
     * @return
     * @see com.fjzxdz.ams.module.debriefing.service.EnvironmentRectifitionService#listByPage(com.fjzxdz.ams.module.debriefing.param.EnvironmentRectifitionParam, cn.fjzxdz.frame.common.Page)
     */
    @Override
    ...
    @Override
    public List<EnvironmentRectifitionChart> getProjectCountByCity(String status, String startTime, String endTime, String userDeptName, String num) {
        String sql = "select COUNT(b.CITY_CODE) count, b.CITY_CODE POINT_CODE,b.CITY_CODE POINT_NAME  " +
        ...
        if (org.apache.commons.lang.StringUtils.isNotEmpty(userDeptName)) {
            sql += " and CITY_CODE = " + SqlUtil.toSqlStr(userDeptName);
        } if (org.apache.commons.lang.StringUtils.isNotEmpty(num)) {
            sql += " and num = " + num;
        }
        sql += " and  CITY_CODE is not null group by  b.CITY_CODE,b.CITY_CODE  ";
    ...
    public List<EnvCancellationAccount> getCancelNumberTimeoutData(String type, String startTime, String endTime) {
        ...
        String sql = "SELECT " +
                ...
                " inner join ENVIRONMEENT_RECTIFITION c on a.RECTIFITION_UUID = c.uuid " +
                " WHERE " +
...
//		}
     ...
     * @param uuid
     * @return
     * @see com.fjzxdz.ams.module.debriefing.service.EnvironmentRectifitionService#getById(java.lang.String)
     */
    @Override
     ...
     *
     * @param rectifition
     * @see com.fjzxdz.ams.module.debriefing.service.EnvironmentRectifitionService#save(com.fjzxdz.ams.module.debriefing.entity.EnvironmentRectifition)
     */
    @Override
    public String save(EnvironmentRectifition rectifition) {
        ...
        if (ToolUtil.isEmpty(rectifition.getUuid())) {
            ...
            if (ToolUtil.isNotEmpty(rectifition.getProjectId())) {
                ...
                if ("100".equals(numOfRoundValue)) {
                    rectifition.setNum(numOfRoundValue);
                    rectifition.setNumOfRoundValue(numOfRoundValue);
                } else {
                    int length = numOfRoundValue.length();
                    List<Map> getNum = simpleDao.getNativeQueryList("SELECT *  FROM COMMON_RELATION_TABLE WHERE RELATION ='NUM_OF_ROUND_ZZ' AND  CODE = '" + (length > 4 ? numOfRoundValue.substring(length - 5, length - 1) : numOfRoundValue) + "'");
     ...
     * @param userDeptName
	 * @return
     * @see com.fjzxdz.ams.module.debriefing.service.EnvironmentRectifitionService#getEcharts(java.util.Date, java.util.Date)
     */
    @Override
    public JSONObject getEcharts(String startTime, String endTime, String userDeptName) {
		if (!org.apache.commons.lang.StringUtils.isNotEmpty(userDeptName)) {
			userDeptName = "";
		}
        List<EnvironmentRectifition> list = rectifitionDao.selectList("from EnvironmentRectifition where mark = ? and numOfRoundValue=? and cityCode like '%"+userDeptName+"%' ", "ZZ", startTime);
...
//		}
     ...
     * @param userDeptName
	 * @return
     * @see EnvironmentRectifitionService#getPieEcharts(String, String, String)
     */
    @SuppressWarnings("unchecked")
    @Override
    public JSONObject getPieEcharts(String startTime, String endTime, String userDeptName) {
		StringBuilder sql = new StringBuilder(" select b.name,a.status,a.UUID from ENVIRONMEENT_RECTIFITION a ");
		sql.append("  LEFT JOIN COMMON_RELATION_TABLE b on a.name=b.UUID where a.mark ='ZZ'  ");
		sql.append(" and a.NUM_OF_ROUND_VALUE = ").append(SqlUtil.toSqlStr(startTime));
		if (org.apache.commons.lang.StringUtils.isNotEmpty(userDeptName)) {
			sql.append(" and CITY_CODE = ").append(SqlUtil.toSqlStr(userDeptName));
		}
     ...
     * @param param
     * @return
     * @see com.fjzxdz.ams.module.debriefing.service.EnvironmentRectifitionService#getCount(com.fjzxdz.ams.module.debriefing.param.EnvironmentRectifitionParam)
     */
    @SuppressWarnings("unchecked")
    @Override
    public JSONObject getCount(EnvironmentRectifitionParam param) {
        ...
        int sum = 0;
        String sql = "SELECT e.STATUS ,COUNT(e.STATUS)  FROM ENVIRONMEENT_RECTIFITION e INNER JOIN  COMMON_RELATION_TABLE  c ON e.NAME = c.UUID where e.mark ='ZZ'";
        String cityCode = param.getCityCode();
        if (org.apache.commons.lang.StringUtils.isNotEmpty(cityCode)) {
            sql += " and CITY_CODE = " + SqlUtil.toSqlStr(cityCode);
        }
        ...
        if (ToolUtil.isNotEmpty(param.getNum())) {
            sql += " and e.num = " + param.getNum() + "";
        }
     ...
     * @param status
     * @return
     * @see com.fjzxdz.ams.module.debriefing.service.EnvironmentRectifitionService#(java.lang.String, java.lang.String)
     */
    @Override
     ...
     * @param page
     * @return
     * @see com.fjzxdz.ams.module.debriefing.service.EnvironmentRectifitionService#getDetail(java.lang.String, cn.fjzxdz.frame.common.Page)
     */
    @Override
     ...
     * @param userDeptName
     * @return
     * @see com.fjzxdz.ams.module.debriefing.service.EnvironmentRectifitionService#(java.lang.String, cn.fjzxdz.frame.common.Page)
     */
    @Override
    public Page<Map<String, Object>> getDecriptionAll(String status, String startTime, String endTime, Page<Map<String, Object>> page, String userDeptName) {
        String sql = "SELECT a.UUID,c.NAME,b.NAME DESCRIBE FROM ENVIRONMEENT_RECTIFITION a INNER JOIN COMMON_RELATION_TABLE b ON a.DESCRIBE=b.UUID INNER JOIN COMMON_RELATION_TABLE c ON a.NAME=c.UUID where mark='ZZ' ";
        sql += " and  STATUS='" + status + "' and a.NUM_OF_ROUND_VALUE = '" + startTime + "' ";
        if (org.apache.commons.lang.StringUtils.isNotEmpty(userDeptName)) {
            sql += " and CITY_CODE = " + SqlUtil.toSqlStr(userDeptName);
        }
     ...
     * @param uuid
     * @return
     * @see com.fjzxdz.ams.module.debriefing.service.EnvironmentRectifitionService#deleteByUuid(java.lang.String)
     */
    @Override
     ...
     *
     * @param rectifition
     * @see com.fjzxdz.ams.module.debriefing.service.EnvironmentRectifitionService#saveRectifition(com.fjzxdz.ams.module.debriefing.entity.EnvironmentRectifition)
     */
    @Override
    public String saveRectifition(EnvironmentRectifition rectifition) {
        ...
        if (!StringUtils.isNull(rectifition.getUuid())) {
            ...
            temp.setAreaCode(rectifition.getAreaCode());
            temp.setCityCode(rectifition.getCityCode());
            temp.setCreateTime(rectifition.getCreateTime());
            temp.setMark("ZZ");
            temp.setDutyDepartment(rectifition.getDutyDepartment());
            temp.setInvolveDepartment(rectifition.getInvolveDepartment());
            temp.setInvolvePerson(rectifition.getInvolvePerson());
            temp.setLeadPerson(rectifition.getLeadPerson());
            temp.setDutyPerson(rectifition.getDutyPerson());
            temp.setDutyPersonPhone(rectifition.getDutyPersonPhone());
            temp.setDutyUnit(rectifition.getDutyUnit());
            temp.setLeadUnit(rectifition.getLeadUnit());
            temp.setMatchUnit(rectifition.getMatchUnit());
            rectifitionDao.update(temp);
        }
        return "";
    ...
    public Page<EnvironmentRectifition> getWornListByPage(EnvironmentRectifitionParam param, Page<EnvironmentRectifition> page) {
        ...
        try {
            ...
            sql.append("        t.city_code AS city_name,");//查询行政区划及所属市区
            sql.append("        t.city_code AS area_name ");//查询行政区划及所属市区
            sql.append(" from environmeent_rectifition t ) a ");
            sql.append("  CROSS JOIN common_relation_table commonrela1_");
            sql.append("        CROSS JOIN common_relation_table commonrela2_ ");
            sql.append("      WHERE a.mark ='ZZ' and a.name = commonrela1_.uuid AND a.describe = commonrela2_.uuid ");
            ...
            if (ObjectUtil.isNotNull(param.getNum())) {
                sql.append(" and a.num = ").append(param.getNum());
            }
    ...
    public List<EnvironmentRectifition> allWornList() {
        ...
        String sql = "SELECT c.NAME AS project_name,D.NAME AS describle_name,A.*\n" +
                ...
                " LEFT JOIN COMMON_RELATION_TABLE c ON A.NAME = c.UUID\n" +
                " LEFT JOIN COMMON_RELATION_TABLE D ON A.DESCRIBE = D.UUID\n" +
                " where A.MARK ='ZZ' AND  ceil(a.TIMELIMIT  - To_date(to_char(sysdate , 'yyyy-mm-dd hh24-mi-ss'),'yyyy-mm-dd hh24-mi-ss')) <= a.worn_time\n" +
    ...
    public void setWarnDay(String warnTime) {
        try {
            StringBuilder sql = new StringBuilder("UPDATE ENVIRONMEENT_RECTIFITION SET WORN_TIME = ? WHERE MARK ='ZZ' ");
     ...
     * @param page
     * @return
     * @see com.fjzxdz.ams.module.debriefing.service.EnvironmentRectifitionService#getProdectDetail(java.lang.String, cn.fjzxdz.frame.common.Page)
     */
    @Override
    ...
    public List getNextRound(String c) {
        StringBuilder sql = new StringBuilder(" SELECT * FROM COMMON_RELATION_TABLE WHERE RELATION ='NUM_OF_ROUND_ZZ' AND  NUM IS NOT NULL ORDER BY NUM DESC ");
