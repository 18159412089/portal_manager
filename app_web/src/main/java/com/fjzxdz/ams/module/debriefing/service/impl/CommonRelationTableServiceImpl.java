...
import com.fjzxdz.ams.module.debriefing.service.CommonRelationTableService;
...
public class CommonRelationTableServiceImpl implements CommonRelationTableService {

    @SuppressWarnings("unused")
    private static Logger logger = LogManager.getLogger(CommonRelationTableServiceImpl.class);
    ...
    public String save(CommonRelationTable commonRelationTable) {
        ...
        if ("NUM_OF_ROUND_ZZ".equals(commonRelationTable.getRelation())) {
    ...
    public List<CommonRelationTable> listNoPage(CommonRelationTableParam param) {
        StringBuilder sql = new StringBuilder(" select * from common_relation_table where 1=1 ");
        sql.append(" and code =  ").append(SqlUtil.toSqlStr(param.getCode()));
        sql.append(" and relation =  ").append(SqlUtil.toSqlStr(param.getRelation()));
        if (!StringUtils.isEmpty(param.getNum())) {
            sql.append(" and num = ").append(param.getNum());
        }
        sql.append(" order by update_Date desc nulls last ");
        List<CommonRelationTable> list = simpleDao.getNativeQueryList(sql.toString(), CommonRelationTable.class);
    ...
    public String delete(String uuid) {
        ...
        if ("NUM_OF_ROUND_ZZ".equals(relationTable.getRelation())) {
            String s = simpleDao.createNativeQuery("select count(1) from ENVIRONMEENT_RECTIFITION where num_of_round_value = ? ", relationTable.getCode()).getResultList().get(0).toString();
            ...
            List<Object[]> list = simpleDao.createNativeQuery("select * from ENVIRONMEENT_RECTIFITION where name=? or describe=? ", uuid, uuid).getResultList();
            if (ToolUtil.isNotEmpty(list)) {
                return "该值被引用，无法删除";
            } else
                commonRelationTableDao.deleteById(uuid);
