...
public class ConfigConnection {
	...
	private ConfigConnection() {
		...
		try {
			String path = ConfigConnection.class.getResource("/").getPath() + "../config_connection.prop";
	...
	private void readConnections(Properties config) {
		try {
			this.minConnections = Integer.valueOf(config.getProperty("minConnections"));
		} catch (Exception e) {
			log.error(Utils.getOriginalMessageFromException(e), e);
		...
		try {
			this.maxConnections = Integer.valueOf(config.getProperty("maxConnections"));
		} catch (Exception e) {
			log.error(Utils.getOriginalMessageFromException(e), e);
	...
	public static String getDataBaseUrl() {
		try {
			String userDecrypted = RSATool.decrypt(ConfigConnection.getInstance().user.replaceAll("_", "="))
					.substring("user".length());
			String passwordDecrypted = RSATool.decrypt(ConfigConnection.getInstance().password.replaceAll("_", "="))
					.substring("password".length());

			return String.format("%s;user=%s;password=%s", ConfigConnection.getInstance().dbUrl, userDecrypted,
					passwordDecrypted);
		} catch (Exception e) {
			e.printStackTrace();
		}
