 ...
 */
@Controller
@RequestMapping("/environment/envCancellationAccount")
@Secured({ "ROLE_USER" })
//@Secured({ "ROLE_ADMIN", "ROLE_ROOT" })
public class EnvCancellationAccountController extends BaseController {

    @Autowired
    private EnvCancellationAccountService envCancellationAccountService;
    @Autowired
    ...
    public ModelAndView index(ModelAndView modelAndView,String authority,String pid) {
    	...
    	modelAndView.setViewName("/moudles/debriefing/envCancellationAccountList");
        return modelAndView;
    }
    ...
    public ModelAndView show(ModelAndView modelAndView,String authority,String pid) {
        modelAndView.addObject("authority", authority);
        modelAndView.addObject("pid", pid);
    	modelAndView.setViewName("/moudles/debriefing/envCancellationAccountIndex");
        return modelAndView;
    }
    ...
    public PageEU<EnvCancellationAccount> list(EnvCancellationAccountParam param, HttpServletRequest request, HttpServletResponse response) {
		...
		sql.append(" select b.name project_Name,  ");
		sql.append("        c.name describle_Name,  ");
		sql.append("        a.uuid,  ");
		sql.append("        a.city_actual_time,  ");
		sql.append("        a.city_estimate_time,  ");
		sql.append("        a.county_actual_time,  ");
		sql.append("        a.county_estimate_time,  ");
		sql.append("        a.profession_actual_time,  ");
		sql.append("        a.profession_estimate_time,  ");
		sql.append("        a.profession_examine_time,  ");
		sql.append("        a.schedule,  ");
		sql.append("        a.time_limit,  ");
		sql.append("        a.rectifition_uuid  ");
		sql.append(" from env_cancellation_account a  ");
		sql.append("          cross join common_relation_table b  ");
		sql.append("          cross join common_relation_table c  ");
		sql.append("          cross join ENVIRONMEENT_RECTIFITION d  ");
		sql.append(" where a.name = b.uuid  ");
		sql.append("   and a.describe = c.uuid  ");
		sql.append("   and a.RECTIFITION_UUID = d.uuid  ");
		String userDeptName = getUserDeptName();
		if (StringUtils.isNotEmpty(userDeptName)) {
			sql.append("   and d.CITY_CODE =   ").append(SqlUtil.toSqlStr(userDeptName));
		}if (StringUtils.isNotEmpty(param.getProjectName())) {
			sql.append("   and b.name  like   ").append(SqlUtil.toSqlStr_like(param.getProjectName()));
		}if (StringUtils.isNotEmpty(param.getDescribleName())) {
			sql.append("   and c.name like   ").append(SqlUtil.toSqlStr_like(param.getDescribleName()));
		}
		sql.append(" order by a.create_date desc  ");

        Page<EnvCancellationAccount> accountPage =  simpleDao.listNativeByPage(sql.toString(), page);;
        //导出
...
//		endTime = CountUtil.getEndDate(endTime,0);
		...
		String userDeptName = getUserDeptName();
		JSONObject result = envCancellationAccountService.getEcharts(startTime,userDeptName);
        return result;
    }
    ...
    public R getDescript(EnvCancellationAccountParam param) {
		String userDeptName = getUserDeptName();
		return envCancellationAccountService.getDescript(param,userDeptName);
    }
