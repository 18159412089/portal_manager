 ...
 */
@Controller
@RequestMapping("/api/pollutionWaterData")
public class PollutionWaterDataApiController extends BaseController {
     ...
     * @return
     */
    @RequestMapping("/getPollutionWaterDataList")
    @ApiValid(value = "350629")
    @ResponseBody
    public R getPollutionWaterDataList(String token,String qymc, HttpServletRequest request) {
        WebServiceMessage msg = new WebServiceMessage();

        String sql = "select QYMC,WRWNAME,YEAR,PFL,LXR,QX,DS,LXDH,ADDRESS,LONGITUDE,LATITUDE from POLLUTION_WATER_DATA where 1=1";
