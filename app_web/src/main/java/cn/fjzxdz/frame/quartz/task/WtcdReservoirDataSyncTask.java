...
@Component("wtcdReservoirDataSyncTask")
public class WtcdReservoirDataSyncTask {

    private static Logger logger = LogManager.getLogger(WtcdReservoirDataSyncTask.class);
    ...
    public void syncData() throws Exception {
        ...
        try {
            ...
            proc = conn.prepareCall("{call P_WTCD_RESERVOIR_DATA(?,?,?,?,?,?)}");
            getData(queryObject, conn, proc, endpoint);
        } catch (Exception e) {
            logger.info("同步水利厅实时水库数据失败:" + e.getMessage());
            throw e;
        } finally {
            if (proc != null) {
                proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
        ...
        String result = (String) call.invoke(new Object[]{"8a1e8dbc5b66b092015bd67c205d6003", "3506", queryObject.toString()});
        // 给方法传递参数，并且调用方法
        JSONObject jsonObject = new JSONObject(result);
        if (jsonObject.getStr("data") == null
                || "".equals(jsonObject.getStr("data"))
                || "null".equals(jsonObject.getStr("data"))) {
            logger.info("同步水利厅实时水库数据为空:" + result);
            return;
        }
        ...
        if (dataArray != null && dataArray.size() > 0) {
            for (Object object : dataArray) {
                ...
                proc.setString(1, getString(tempObject.get("W")));
                proc.setString(2, getString(tempObject.get("RKSJ")));
                proc.setString(3, getString(tempObject.get("STCD")));
                proc.setString(4, getString(tempObject.get("UPDATETIME_RJWA")));
                proc.setString(5, getString(tempObject.get("YMDHM")));
                proc.setString(6, getString(tempObject.get("ZI")));
                proc.addBatch();
            }
            proc.executeBatch();
            conn.commit();
