...
@Component("airMonitorPointSyncTask")
public class AirMonitorPointSyncTask {

    private static Logger logger = LogManager.getLogger(AirMonitorPointSyncTask.class);
...
//		queryObject.put("params", paramArray);
		...
		try {
			...
			proc = conn.prepareCall("{call P_AIR_MONITOR_POINT(?,?,?,?,?,?,?,?,?,?)}");
			getData(queryObject,conn,proc,endpoint);
		} catch (Exception e) {
			logger.info("同步空气站点数据失败:"+e.getMessage());
			throw e;
		} finally {
			if(proc!=null) {
				proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
		...
		String result = (String) call.invoke(new Object[] { "4028838963ca04410163fd487a435de4","3506",queryObject.toString() });  
		// 给方法传递参数，并且调用方法  
		...
		if(dataArray!=null&&dataArray.size()>0) {
			for (Object object : dataArray) {
				...
				proc.setString(1, getString(tempObject.get("POINTCODE")));
				proc.setString(2, getString(tempObject.get("POINTNAME")));
				proc.setString(3, getString(tempObject.get("POINTTYPE")));
				proc.setString(4, getString(tempObject.get("POINTADDRESS")));
				proc.setString(5, getString(tempObject.get("CODE_AIRLEVEL")));
				proc.setString(6, getString(tempObject.get("CODE_REGION")));
				proc.setString(7, getString(tempObject.get("REGIONNAME")));
				proc.setBigDecimal(8, getNumber(tempObject.get("YEARNUMBER")));
				proc.setBigDecimal(9, getNumber(tempObject.get("LONGITUDE")));
				proc.setBigDecimal(10, getNumber(tempObject.get("LATITUDE")));
				proc.addBatch();
			}
			proc.executeBatch();
			conn.commit();
