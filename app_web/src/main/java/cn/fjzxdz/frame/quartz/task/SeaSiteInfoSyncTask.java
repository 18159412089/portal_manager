...
@Component("seaSiteInfoSyncTask")
public class SeaSiteInfoSyncTask {

    private static Logger logger = LogManager.getLogger(SeaSiteInfoSyncTask.class);
    ...
    public void syncData() throws Exception {
        ...
        try {
            ...
            proc = conn.prepareCall("{call P_SEA_SITE_INFO(?,?,?,?,?,?,?,?,?)}");
            getData(queryObject, conn, proc, endpoint);
        } catch (Exception e) {
            logger.info("同步海洋与渔业点位信息失败:" + e.getMessage());
            throw e;
        } finally {
            if (proc != null) {
                proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
        ...
        String result = (String) call.invoke(new Object[]{"402883896734647801686027724c1358", "3506", queryObject.toString()});
        // 给方法传递参数，并且调用方法
        JSONObject jsonObject = new JSONObject(result);
        if (jsonObject.getStr("data") == null
                || "".equals(jsonObject.getStr("data"))
                || "null".equals(jsonObject.getStr("data"))) {
            logger.info("同步海洋与渔业点位信息为空:" + result);
            return;
        }
        ...
        if (dataArray != null && dataArray.size() > 0) {
            for (Object object : dataArray) {
                ...
                proc.setString(1, getString(tempObject.get("ZDBH")));
                proc.setString(2, getString(tempObject.get("YZDBH")));
                proc.setString(3, getString(tempObject.get("ZDMC")));
                proc.setBigDecimal(4, getNumber(tempObject.get("JD")));
                proc.setBigDecimal(5, getNumber(tempObject.get("WD")));
                proc.setBigDecimal(6, getNumber(tempObject.get("XZQHBM")));
                proc.setString(7, getString(tempObject.get("XZQHMC")));
                proc.setString(8, getString(tempObject.get("JCPC")));
                proc.setTimestamp(9, getTimestamp(tempObject.get("UPDATETIME_RJWA")));
                proc.addBatch();
            }
            proc.executeBatch();
            conn.commit();
