...
@Component("hydPositionInfoSyncTask")
public class HydPositionInfoSyncTask {

    private static Logger logger = LogManager.getLogger(HydPositionInfoSyncTask.class);
    ...
    public void syncData() throws Exception {
        ...
        try {
            ...
            proc = conn.prepareCall("{call P_HYD_POSITION_INFO(?,?,?,?,?)}");
            getData(queryObject, conn, proc, endpoint);
        } catch (Exception e) {
            logger.info("同步水电站点位基本信息失败:" + e.getMessage());
            throw e;
        } finally {
            if (proc != null) {
                proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
        ...
        String result = (String) call.invoke(new Object[]{"8a1e8dbc5b66b092015bd693b4fd603b", "3506", queryObject.toString()});
        // 给方法传递参数，并且调用方法
        JSONObject jsonObject = new JSONObject(result);
        if (jsonObject.getStr("data") == null
                || "".equals(jsonObject.getStr("data"))
                || "null".equals(jsonObject.getStr("data"))) {
            logger.info("同步水电站点位基本信息为空:" + result);
            return;
        }
        ...
        if (dataArray != null && dataArray.size() > 0) {
            for (Object object : dataArray) {
                ...
                proc.setString(1, getString(tempObject.get("IS_USED")));
                proc.setBigDecimal(2, getNumber(tempObject.get("EQP_ID")));
                proc.setBigDecimal(3, getNumber(tempObject.get("HYDROPOWER_ID")));
                proc.setString(4, getString(tempObject.get("EQP_NAME")));
                proc.setBigDecimal(5, getNumber(tempObject.get("CYC")));
                proc.addBatch();
            }
            proc.executeBatch();
            conn.commit();
