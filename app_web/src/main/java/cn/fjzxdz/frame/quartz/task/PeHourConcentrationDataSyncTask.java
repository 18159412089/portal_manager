...
@Component("peHourConcentrationDataSyncTask")
public class PeHourConcentrationDataSyncTask {

    private static Logger logger = LogManager.getLogger(PeHourConcentrationDataSyncTask.class);
...
//		String startTime = redisUtils.get("sync:peHourConcentrationData")==null?sdfTime.format(DateUtil.addDate(new Date(), -10)):redisUtils.get("sync:peHourConcentrationData");
		paramObject.put("Value", sdfTime.format(DateUtil.addDate(new Date(), -1)));
...
//		paramArray.add(paramObject2);
		...
		try {
			...
			proc = conn.prepareCall("{call P_PE_CON_HOUR_DATA(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
			getData(queryObject,conn,proc,endpoint);
//			redisUtils.set("sync:peHourConcentrationData", nowTime,-1);
		} catch (Exception e) {
			logger.info("同步污染源小时浓度监测数据失败:"+e.getMessage());
			throw e;
		} finally {
			if(proc!=null) {
				proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
		...
		String result = (String) call.invoke(new Object[] { "40288389623c746a0162426dcbc4032e","3506",queryObject.toString() });  
		// 给方法传递参数，并且调用方法  
		...
		if(dataArray!=null&&dataArray.size()>0) {
			for (Object object : dataArray) {
				...
				proc.setBigDecimal(8, getNumber(tempObject.get("OUTPUT_AVG")));
				proc.setString(9, getString(tempObject.get("OUTFALL_TYPE")));
				proc.setString(10, getString(tempObject.get("IS_MEASURE")));
				proc.setBigDecimal(11, getNumber(tempObject.get("DATA_COUNT")));
				proc.setString(12, getString(tempObject.get("PERSONAL_IDENTIFICATION")));
				proc.setString(13, getString(tempObject.get("REMARK")));
				proc.setString(14, getString(tempObject.get("REPORT_TYPE")));
				proc.setTimestamp(15, getTimestamp(tempObject.get("INSERT_TIME")));
				proc.addBatch();
			}
			proc.executeBatch();
			conn.commit();
