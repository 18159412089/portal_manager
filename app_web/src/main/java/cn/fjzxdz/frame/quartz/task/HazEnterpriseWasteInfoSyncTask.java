...
@Component("hazEnterpriseWasteInfoSyncTask")
public class HazEnterpriseWasteInfoSyncTask {

    private static Logger logger = LogManager.getLogger(HazEnterpriseWasteInfoSyncTask.class);
    ...
    public void syncData() throws Exception {
        ...
        try {
            ...
            proc = conn.prepareCall("{call P_HAZ_ENTERPRISE_WASTE_INFO(?,?,?,?,?,?,?,?,?,?)}");
            getData(queryObject, conn, proc, endpoint);
        } catch (Exception e) {
            logger.info("同步企业产废贮存信息失败:" + e.getMessage());
            throw e;
        } finally {
            if (proc != null) {
                proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
        ...
        String result = (String) call.invoke(new Object[]{"402883896734647801686f2b2fb0339b", "3506", queryObject.toString()});
        // 给方法传递参数，并且调用方法
        JSONObject jsonObject = new JSONObject(result);
        if (jsonObject.getStr("data") == null
                || "".equals(jsonObject.getStr("data"))
                || "null".equals(jsonObject.getStr("data"))) {
            logger.info("同步企业产废贮存信息为空:" + result);
            return;
        }
        ...
        if (dataArray != null && dataArray.size() > 0) {
            for (Object object : dataArray) {
                ...
                proc.setString(1, getString(tempObject.get("STORAGE_NAME")));
                proc.setString(2, getString(tempObject.get("UNIT")));
                proc.setString(3, getString(tempObject.get("ENTNAME")));
                proc.setString(4, getString(tempObject.get("WASTE_NAME")));
                proc.setString(5, getString(tempObject.get("ENTERPRISE_ID")));
                proc.setTimestamp(6, getTimestamp(tempObject.get("UPDATETIME")));
                proc.setString(7, getString(tempObject.get("QUANTITY")));
                proc.setString(8, getString(tempObject.get("CREATED_TIME")));
                proc.setString(9, getString(tempObject.get("CODE")));
                proc.setString(10, getString(tempObject.get("ENTERPRISE_ATTRIBUTE")));
                proc.addBatch();
            }
            proc.executeBatch();
            conn.commit();
