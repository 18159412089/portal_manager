...
@Component("wtMonitorPointSyncTask")
public class WtMonitorPointSyncTask {

    private static Logger logger = LogManager.getLogger(WtMonitorPointSyncTask.class);
...
//		queryObject.put("params", paramArray);
		...
		try {
			...
			proc = conn.prepareCall("{call P_WT_MONITOR_POINT(?,?,?,?,?,?,?,?)}");
			getData(queryObject,conn,proc,endpoint);
		} catch (Exception e) {
			logger.info("同步水检测站点数据失败:"+e.getMessage());
			throw e;
		} finally {
			if(proc!=null) {
				proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
		...
		String result = (String) call.invoke(new Object[] { "40288389632efb6301636d547a3f5149","3506",queryObject.toString() });  
		// 给方法传递参数，并且调用方法  
		...
		if(dataArray!=null&&dataArray.size()>0) {
			for (Object object : dataArray) {
				...
				proc.setString(3, getString(tempObject.get("CODE_WSYSTEM")));
				proc.setString(4, getString(tempObject.get("WSYSTEMNAME")));
				proc.setString(5, getString(tempObject.get("CODE_REGION")));
				proc.setString(6, getString(tempObject.get("REGIONNAME")));
				proc.setBigDecimal(7, getNumber(tempObject.get("LONGITUDE")));
