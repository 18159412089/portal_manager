...
@Component("peHourFlowDataSyncTask")
public class PeHourFlowDataSyncTask {

    private static Logger logger = LogManager.getLogger(PeHourFlowDataSyncTask.class);
...
//		String startTime = redisUtils.get("sync:peHourFlowData")==null?sdfTime.format(DateUtil.addDate(new Date(), -10)):redisUtils.get("sync:peHourFlowData");
...
//		paramArray.add(paramObject2);
		...
		try {
			...
			proc = conn.prepareCall("{call P_PE_HOUR_FLUX_DATA(?,?,?,?,?,?,?,?,?,?,?,?)}");
			getData(queryObject,conn,proc,endpoint);
//			redisUtils.set("sync:peHourFlowData", nowTime,-1);
		} catch (Exception e) {
			logger.info("同步污染源小时流量监测数据失败:"+e.getMessage());
			throw e;
		} finally {
			if(proc!=null) {
				proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
		...
		String result = (String) call.invoke(new Object[] { "40288389623c746a016242672cac02e4","3506",queryObject.toString() });  
		// 给方法传递参数，并且调用方法  
		...
		if(dataArray!=null&&dataArray.size()>0) {
			for (Object object : dataArray) {
				...
				proc.setBigDecimal(4, getNumber(tempObject.get("FLUX_MIN")));
				proc.setBigDecimal(5, getNumber(tempObject.get("FLUX_MAX")));
				proc.setBigDecimal(6, getNumber(tempObject.get("FLUX_AVG")));
				...
				proc.setString(10, getString(tempObject.get("STOP_PRODUCTION")));
				proc.setTimestamp(11, getTimestamp(tempObject.get("INSERT_TIME")));
				proc.setString(12, getString(tempObject.get("REPORT_TYPE")));
				proc.addBatch();
			}
			proc.executeBatch();
			conn.commit();
