...
@Component("airSiteHourDataSyncTask")
public class AirSiteHourDataSyncTask {

    private static Logger logger = LogManager.getLogger(AirSiteHourDataSyncTask.class);
    ...
    public void syncData() throws Exception {
        ...
        paramObject.put("FieldName", "CHECKTIME");
        ...
        try {
            ...
            logger.info("同步空气站点气象小时数据:" + e.getMessage());
            throw e;
        } finally {
            if (proc != null) {
                proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
        ...
        String result = (String) call.invoke(new Object[]{"4028838967346478016879aaab1324f2", "3506", queryObject.toString()});
        // 给方法传递参数，并且调用方法
        JSONObject jsonObject = new JSONObject(result);
        if (jsonObject.getStr("data") == null
                || "".equals(jsonObject.getStr("data"))
                || "null".equals(jsonObject.getStr("data"))) {
            logger.info("同步空气站点气象小时为空:" + result);
            return;
        }
        ...
        if (dataArray != null && dataArray.size() > 0) {
            for (Object object : dataArray) {
                ...
                proc.setString(1, getString(tempObject.get("WINDDIRT")));
                proc.setString(2, getString(tempObject.get("AREANAME")));
                proc.setString(3, getString(tempObject.get("STATE")));
                proc.setString(4, getString(tempObject.get("RAINFALL")));
                proc.setString(5, getString(tempObject.get("POSCODE")));
                proc.setString(6, getString(tempObject.get("WINDSPD")));
                proc.setString(7, getString(tempObject.get("POTCODE")));
                proc.setString(8, getString(tempObject.get("SAREANAME")));
                proc.setString(9, getString(tempObject.get("TEMP")));
                proc.setString(10, getString(tempObject.get("PRESSURE")));
                proc.setString(11, getString(tempObject.get("POTNAME")));
                proc.setString(12, getString(tempObject.get("HUMI")));
                proc.setTimestamp(13, getTimestamp(tempObject.get("CHECKTIME")));
                proc.addBatch();
            }
            proc.executeBatch();
            conn.commit();
