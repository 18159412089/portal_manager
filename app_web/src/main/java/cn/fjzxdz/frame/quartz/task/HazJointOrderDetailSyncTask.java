...
@Component("hazJointOrderDetailSyncTask")
public class HazJointOrderDetailSyncTask {

    private static Logger logger = LogManager.getLogger(HazJointOrderDetailSyncTask.class);
    ...
    public void syncData() throws Exception {
        ...
        paramObject.put("FieldName", "UPDATETIME");//监测时间
        ...
        try {
            ...
            proc = conn.prepareCall("{call P_HAZ_JOINT_ORDER_DETAIL(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
            getData(queryObject, conn, proc, endpoint);
        } catch (Exception e) {
            logger.info("同步联单详细信息失败:" + e.getMessage());
            throw e;
        } finally {
            if (proc != null) {
                proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
        ...
        String result = (String) call.invoke(new Object[]{"40288389673464780168700a48eb384f", "3506", queryObject.toString()});
        // 给方法传递参数，并且调用方法
        JSONObject jsonObject = new JSONObject(result);
        if (jsonObject.getStr("data") == null
                || "".equals(jsonObject.getStr("data"))
                || "null".equals(jsonObject.getStr("data"))) {
            logger.info("同步联单详细信息为空:" + result);
            return;
        }
        ...
        if (dataArray != null && dataArray.size() > 0) {
            for (Object object : dataArray) {
                ...
                proc.setString(1, getString(tempObject.get("GUIGEXINGHAO")));
                proc.setString(2, getString(tempObject.get("CAIZHI")));
                proc.setString(3, getString(tempObject.get("WXCF")));
                proc.setBigDecimal(4, getNumber(tempObject.get("ZYL")));
                proc.setBigDecimal(5, getNumber(tempObject.get("RONGQISL")));
                proc.setString(6, getString(tempObject.get("SUCHENG")));
                proc.setString(7, getString(tempObject.get("WXTX")));
                proc.setString(8, getString(tempObject.get("RONGQILX")));
                proc.setString(9, getString(tempObject.get("CZFS_NAME")));
                proc.setString(10, getString(tempObject.get("ZYLD_ID")));
                proc.setString(11, getString(tempObject.get("CZ_JSL")));
                proc.setTimestamp(12, getTimestamp(tempObject.get("UPDATETIME")));
                proc.setString(13, getString(tempObject.get("CODE")));
                proc.setString(14, getString(tempObject.get("DANWEI")));
                proc.addBatch();
            }
            proc.executeBatch();
            conn.commit();
