...
@Component("wtcdRainfallDataSyncTask")
public class WtcdRainfallDataSyncTask {

    private static Logger logger = LogManager.getLogger(WtcdRainfallDataSyncTask.class);
    ...
    public void syncData() throws Exception {
        ...
        try {
            ...
            proc = conn.prepareCall("{call P_WTCD_RAINFALL_DATA(?,?,?,?,?)}");
            getData(queryObject, conn, proc, endpoint);
        } catch (Exception e) {
            logger.info("同步水利厅实时雨量数据失败:" + e.getMessage());
            throw e;
        } finally {
            if (proc != null) {
                proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
        ...
        String result = (String) call.invoke(new Object[]{"8a1e8dbc5b66b092015bd67fa8ac600c", "3506", queryObject.toString()});
        // 给方法传递参数，并且调用方法
        ...
        if (dataArray != null && dataArray.size() > 0) {
            for (Object object : dataArray) {
                ...
                proc.setTimestamp(1, getTimestamp(tempObject.get("RKSJ")));
                proc.setString(2, getString(tempObject.get("STCD")));
                proc.setString(3, getString(tempObject.get("UPDATETIME_RJWA")));
                proc.setBigDecimal(4, getNumber(tempObject.get("DTRN")));
