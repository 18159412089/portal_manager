...
@Component("hydDevExitDataSyncTask")
public class HydDevExitDataSyncTask {

    private static Logger logger = LogManager.getLogger(HydDevExitDataSyncTask.class);
    ...
    public void syncData() throws Exception {
        ...
        try {
            ...
            proc = conn.prepareCall("{call P_HYD_DEV_EXIT_DATA(?,?,?,?,?,?,?)}");
            getData(queryObject, conn, proc, endpoint);
        } catch (Exception e) {
            logger.info("同步水电站设备出口失败:" + e.getMessage());
            throw e;
        } finally {
            if (proc != null) {
                proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
        ...
        String result = (String) call.invoke(new Object[]{"8a1e8dbc5b66b092015bd694f1886047", "3506", queryObject.toString()});
        // 给方法传递参数，并且调用方法
        JSONObject jsonObject = new JSONObject(result);
        if (jsonObject.getStr("data") == null
                || "".equals(jsonObject.getStr("data"))
                || "null".equals(jsonObject.getStr("data"))) {
            logger.info("同步水电站设备出口为空:" + result);
            return;
        }
        ...
        if (dataArray != null && dataArray.size() > 0) {
            for (Object object : dataArray) {
                ...
                proc.setBigDecimal(1, getNumber(tempObject.get("MIN_FLOW")));
                proc.setBigDecimal(2, getNumber(tempObject.get("EQP_ID")));
                proc.setBigDecimal(3, getNumber(tempObject.get("OUTPUT_ID")));
                proc.setBigDecimal(4, getNumber(tempObject.get("HYDROPOWER_ID")));
                proc.setString(5, getString(tempObject.get("STATUS")));
                proc.setString(6, getString(tempObject.get("OUTPUT_NAME")));
                proc.setString(7, getString(tempObject.get("OUTPUT_CODE")));
