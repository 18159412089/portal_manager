...
@Component("riskProductAccessoriesDataSyncTask")
public class RiskProductAccessoriesDataSyncTask {

    private static Logger logger = LogManager.getLogger(RiskProductAccessoriesDataSyncTask.class);
    ...
    public void syncData() throws Exception {
        ...
        try {
            ...
            proc = conn.prepareCall("{call P_RISK_PRODUCT_ACCESSORIES_D(?,?,?,?,?,?,?,?,?,?,?)}");
            getData(queryObject, conn, proc, endpoint);
        } catch (Exception e) {
            logger.info("同步产品及辅料基本情况信息失败:" + e.getMessage());
            throw e;
        } finally {
            if (proc != null) {
                proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
        ...
        String result = (String) call.invoke(new Object[]{"402883896734647801687976837523ff", "3506", queryObject.toString()});
        // 给方法传递参数，并且调用方法
        JSONObject jsonObject = new JSONObject(result);
        if (jsonObject.getStr("data") == null
                || "".equals(jsonObject.getStr("data"))
                || "null".equals(jsonObject.getStr("data"))) {
            logger.info("同步产品及辅料基本情况信息为空:" + result);
            return;
        }
        ...
        if (dataArray != null && dataArray.size() > 0) {
            for (Object object : dataArray) {
                ...
                proc.setString(1, getString(tempObject.get("PRODUCTSNAME")));
                proc.setString(2, getString(tempObject.get("GUID")));
                proc.setString(3, getString(tempObject.get("ACTUALLYPRODUCE")));
                proc.setBigDecimal(4, getNumber(tempObject.get("DESIGNCAPACITY")));
                proc.setString(5, getString(tempObject.get("CAS")));
                proc.setString(6, getString(tempObject.get("ID")));
                proc.setString(7, getString(tempObject.get("MATERIALSNAME")));
                proc.setTimestamp(8, getTimestamp(tempObject.get("SUBMITTEDTIME")));
                proc.setString(9, getString(tempObject.get("FK_TRANSPORTMODE")));
                proc.setTimestamp(10, getTimestamp(tempObject.get("UPDATETIME")));
                proc.setBigDecimal(11, getNumber(tempObject.get("CONSUMPTION")));
                proc.addBatch();
            }
            proc.executeBatch();
            conn.commit();
