...
@Component("radSourceAccountSyncTask")
public class RadSourceAccountSyncTask {

    private static Logger logger = LogManager.getLogger(RadSourceAccountSyncTask.class);
    ...
    public void syncData() throws Exception {
        ...
        try {
            ...
            proc = conn.prepareCall("{call P_RAD_SOURCE_ACCOUNT(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
            getData(queryObject, conn, proc, endpoint);
        } catch (Exception e) {
            logger.info("同步放射源台账失败:" + e.getMessage());
            throw e;
        } finally {
            if (proc != null) {
                proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
        ...
        String result = (String) call.invoke(new Object[]{"8a1e8dbc5b66b092015bd78ffc6d61dc", "3506", queryObject.toString()});
        // 给方法传递参数，并且调用方法
        JSONObject jsonObject = new JSONObject(result);
        if (jsonObject.getStr("data") == null
                || "".equals(jsonObject.getStr("data"))
                || "null".equals(jsonObject.getStr("data"))) {
            logger.info("同步放射源台账为空:" + result);
            return;
        }
        ...
        if (dataArray != null && dataArray.size() > 0) {
            for (Object object : dataArray) {
                ...
                proc.setString(1, getString(tempObject.get("SHR")));
                proc.setString(2, getString(tempObject.get("ENTERID")));
                proc.setString(3, getString(tempObject.get("LY")));
                proc.setString(4, getString(tempObject.get("CS")));
                proc.setString(5, getString(tempObject.get("PKID")));
                proc.setString(6, getString(tempObject.get("UPDATETIME_RJWA")));
                proc.setString(7, getString(tempObject.get("CCRQ")));
                proc.setString(8, getString(tempObject.get("SHRQ")));
                proc.setString(9, getString(tempObject.get("QXSHRQ")));
                proc.setString(10, getString(tempObject.get("FSYYT")));
                proc.setString(11, getString(tempObject.get("BZ")));
                proc.setString(12, getString(tempObject.get("QXSHR")));
                proc.setString(13, getString(tempObject.get("HSMC")));
                proc.setString(14, getString(tempObject.get("QX")));
                proc.setString(15, getString(tempObject.get("CCHD")));
                proc.setString(16, getString(tempObject.get("BH")));
                proc.setString(17, getString(tempObject.get("FSYLB")));
                proc.setString(18, getString(tempObject.get("SFLSTZ")));
                proc.setString(19, getString(tempObject.get("FSYBM")));
                proc.setString(20, getString(tempObject.get("ZT")));
                proc.addBatch();
            }
            proc.executeBatch();
            conn.commit();
