...
@Component("hazTransferCarInfoSyncTask")
public class HazTransferCarInfoSyncTask {

    private static Logger logger = LogManager.getLogger(HazTransferCarInfoSyncTask.class);
    ...
    public void syncData() throws Exception {
        ...
        paramObject.put("FieldName", "UPDATETIME_RJWA");//监测时间
        ...
        try {
            ...
            proc = conn.prepareCall("{call P_HAZ_TRANSFER_CAR_INFO(?,?,?,?,?,?,?,?,?,?,?,?,?)}");
            getData(queryObject, conn, proc, endpoint);
        } catch (Exception e) {
            logger.info("同步危废转移车辆信息失败:" + e.getMessage());
            throw e;
        } finally {
            if (proc != null) {
                proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
        ...
        String result = (String) call.invoke(new Object[]{"8a1e8dbc5b66b092015bf628f20d0de3", "3506", queryObject.toString()});
        // 给方法传递参数，并且调用方法
        JSONObject jsonObject = new JSONObject(result);
        if (jsonObject.getStr("data") == null
                || "".equals(jsonObject.getStr("data"))
                || "null".equals(jsonObject.getStr("data"))) {
            logger.info("同步危废转移车辆信息为空:" + result);
            return;
        }
        ...
        if (dataArray != null && dataArray.size() > 0) {
            for (Object object : dataArray) {
                ...
                proc.setBigDecimal(1, getNumber(tempObject.get("DIRECTION")));
                proc.setString(2, getString(tempObject.get("COMPANY")));
                proc.setString(3, getString(tempObject.get("LOCATION")));
                proc.setTimestamp(4, getTimestamp(tempObject.get("TIME")));
                proc.setString(5, getString(tempObject.get("VNO")));
                proc.setBigDecimal(6, getNumber(tempObject.get("MOM_ID")));
                proc.setTimestamp(7, getTimestamp(tempObject.get("UPDATETIME_RJWA")));
                proc.setBigDecimal(8, getNumber(tempObject.get("SPEED")));
                proc.setString(9, getString(tempObject.get("VCOLOR")));
                proc.setString(10, getString(tempObject.get("STATUS")));
                proc.setBigDecimal(11, getNumber(tempObject.get("Y")));
                proc.setBigDecimal(12, getNumber(tempObject.get("X")));
                proc.setString(13, getString(tempObject.get("TYPE")));
                proc.addBatch();
            }
            proc.executeBatch();
            conn.commit();
