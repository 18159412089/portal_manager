...
@Component("hazJointOrderInfoSyncTask")
public class HazJointOrderInfoSyncTask {

    private static Logger logger = LogManager.getLogger(HazJointOrderInfoSyncTask.class);
    ...
    public void syncData() throws Exception {
        ...
        try {
            ...
            proc = conn.prepareCall("{call P_HAZ_JOINT_ORDER_INFO(?,?,?,?,?,?,?,?,?,?,?,?,?)}");
            getData(queryObject, conn, proc, endpoint);
        } catch (Exception e) {
            logger.info("同步联单转移联单信息失败:" + e.getMessage());
            throw e;
        } finally {
            if (proc != null) {
                proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
        ...
        String result = (String) call.invoke(new Object[]{"402883896734647801686fd9cf943752", "3506", queryObject.toString()});
        // 给方法传递参数，并且调用方法
        JSONObject jsonObject = new JSONObject(result);
        if (jsonObject.getStr("data") == null
                || "".equals(jsonObject.getStr("data"))
                || "null".equals(jsonObject.getStr("data"))) {
            logger.info("同步联单转移联单信息为空:" + result);
            return;
        }
        ...
        if (dataArray != null && dataArray.size() > 0) {
            for (Object object : dataArray) {
                ...
                proc.setString(1, getString(tempObject.get("CHUCHANG")));
                proc.setTimestamp(2, getTimestamp(tempObject.get("CCSJ")));
                proc.setString(3, getString(tempObject.get("CONFIRM_REMARK")));
                proc.setString(4, getString(tempObject.get("LDZT")));
                proc.setString(5, getString(tempObject.get("YSQY_DRIVER")));
                proc.setString(6, getString(tempObject.get("QRCJSJ")));
                proc.setString(7, getString(tempObject.get("ZYLD_ID")));
                proc.setString(8, getString(tempObject.get("ID")));
                proc.setTimestamp(9, getTimestamp(tempObject.get("QRSJ")));
                proc.setString(10, getString(tempObject.get("QRCJ_REMARK")));
                proc.setTimestamp(11, getTimestamp(tempObject.get("CJSJ")));
                proc.setTimestamp(12, getTimestamp(tempObject.get("UPDATETIME")));
                proc.setString(13, getString(tempObject.get("LDBH")));
                proc.addBatch();
            }
            proc.executeBatch();
            conn.commit();
