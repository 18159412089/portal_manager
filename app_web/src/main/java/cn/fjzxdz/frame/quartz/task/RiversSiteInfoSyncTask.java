...
@Component("riversSiteInfoSyncTask")
public class RiversSiteInfoSyncTask {

    private static Logger logger = LogManager.getLogger(RiversSiteInfoSyncTask.class);
...
//		paramArray.add(paramObject);
        ...
        try {
            ...
            proc = conn.prepareCall("{call P_RIVERS_SITE_INFO(?,?,?,?,?,?,?,?)}");
            getData(queryObject, conn, proc, endpoint);
        } catch (Exception e) {
            logger.info("同步入海河流点位信息-新失败:" + e.getMessage());
            throw e;
        } finally {
            if (proc != null) {
                proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
        ...
        String result = (String) call.invoke(new Object[]{"40288389632efb6301636cec2ace4774", "3506", queryObject.toString()});
        // 给方法传递参数，并且调用方法
        JSONObject jsonObject = new JSONObject(result);
        if (jsonObject.getStr("data") == null
                || "".equals(jsonObject.getStr("data"))
                || "null".equals(jsonObject.getStr("data"))) {
            logger.info("同步入海河流点位信息-新为空:" + result);
            return;
        }
        ...
        if (dataArray != null && dataArray.size() > 0) {
            for (Object object : dataArray) {
                ...
                proc.setString(2, getString(tempObject.get("CODE_REGION")));
                proc.setString(3, getString(tempObject.get("LONGITUDE")));
                proc.setString(4, getString(tempObject.get("WSYSTEMNAME")));
                proc.setString(5, getString(tempObject.get("REGIONNAME")));
                proc.setString(6, getString(tempObject.get("CODE_WSYSTEM")));
                proc.setString(7, getString(tempObject.get("LATITUDE")));
                proc.setString(8, getString(tempObject.get("POINTNAME")));
