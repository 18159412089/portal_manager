...
@Component("riversEvaluateDataSyncTask")
public class RiversEvaluateDataSyncTask {

    private static Logger logger = LogManager.getLogger(RiversEvaluateDataSyncTask.class);
...
//		paramObject.put("FieldName", "MONTHNUMBER");
...
//		paramArray.add(paramObject);
        ...
        try {
            ...
            proc = conn.prepareCall("{call P_RIVERS_EVALUATE_DATA(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
            getData(queryObject, conn, proc, endpoint);
        } catch (Exception e) {
            logger.info("同步入海河流水质评价结果-新失败:" + e.getMessage());
            throw e;
        } finally {
            if (proc != null) {
                proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
        ...
        String result = (String) call.invoke(new Object[]{"40288389632efb6301636ce99cd94735", "3506", queryObject.toString()});
        // 给方法传递参数，并且调用方法
        JSONObject jsonObject = new JSONObject(result);
        if (jsonObject.getStr("data") == null
                || "".equals(jsonObject.getStr("data"))
                || "null".equals(jsonObject.getStr("data"))) {
            logger.info("同步入海河流水质评价结果-新为空:" + result);
            return;
        }
        ...
        if (dataArray != null && dataArray.size() > 0) {
            for (Object object : dataArray) {
                ...
                proc.setBigDecimal(1, getNumber(tempObject.get("POINTCODE")));
                proc.setString(2, getString(tempObject.get("CODE_WATERQUALITYLEVELTARGET")));
                proc.setString(3, getString(tempObject.get("CODE_WATERQUALITYLEVEL")));
                proc.setString(4, getString(tempObject.get("YEARNUMBER")));
                proc.setString(5, getString(tempObject.get("REGIONNAME")));
                proc.setString(6, getString(tempObject.get("PRIMPOLLUTE")));
                proc.setString(7, getString(tempObject.get("TASKWATERS")));
                proc.setString(8, getString(tempObject.get("WATERNAME")));
                proc.setString(9, getString(tempObject.get("WATERQUALITYLEVELNAME")));
                proc.setString(10, getString(tempObject.get("CODE_REGION")));
                proc.setString(11, getString(tempObject.get("WATERCODE")));
                proc.setString(12, getString(tempObject.get("MONTHNUMBER")));
                proc.setString(13, getString(tempObject.get("WATERQUALITYLEVELTARGETNAME")));
                proc.setString(14, getString(tempObject.get("POINTNAME")));
