...
@Component("peCommonCodeSyncTask")
public class PeCommonCodeSyncTask {

    private static Logger logger = LogManager.getLogger(PeCommonCodeSyncTask.class);
...
//		queryObject.put("params", paramArray);
		...
		try {
			...
			proc = conn.prepareCall("{call P_PE_COMMON_CODE(?,?,?,?,?,?,?)}");
			getData(queryObject,conn,proc,endpoint);
		} catch (Exception e) {
			logger.info("同步污染源数据字典失败:"+e.getMessage());
			throw e;
		} finally {
			if(proc!=null) {
				proc.close();
