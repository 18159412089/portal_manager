...
@Component("wtMnPointSyncTask")
public class WtMnPointSyncTask {

    private static Logger logger = LogManager.getLogger(WtMnPointSyncTask.class);
...
//		queryObject.put("params", paramArray);
		...
		try {
			...
			proc = conn.prepareCall("{call P_WT_MN_POINT(?,?,?,?,?,?,?,?)}");
			getData(queryObject,conn,proc,endpoint);
		} catch (Exception e) {
			logger.info("同步国家自动水检测断面数据失败:"+e.getMessage());
			throw e;
		} finally {
			if(proc!=null) {
				proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
		...
		String result = (String) call.invoke(new Object[] { "4028838966d006300166e6ba639254a5","3506",queryObject.toString() });  
		// 给方法传递参数，并且调用方法  
		...
		if(dataArray!=null&&dataArray.size()>0) {
			for (Object object : dataArray) {
				...
				proc.setString(1, getString(tempObject.get("MN")));
				proc.setString(2, getString(tempObject.get("MNNAME")));
				proc.setString(3, getString(tempObject.get("AREAID")));
				proc.setString(4, getString(tempObject.get("AREANAME")));
				proc.setString(5, getString(tempObject.get("RIVERID")));
				proc.setString(6, getString(tempObject.get("RIVERNAME")));
				proc.setBigDecimal(7, getNumber(tempObject.get("LNG")));
				proc.setBigDecimal(8, getNumber(tempObject.get("LAT")));
				proc.addBatch();
			}
			proc.executeBatch();
			conn.commit();
