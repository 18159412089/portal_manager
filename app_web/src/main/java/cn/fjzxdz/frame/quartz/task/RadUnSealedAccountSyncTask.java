...
@Component("radUnSealedAccountSyncTask")
public class RadUnSealedAccountSyncTask {

    private static Logger logger = LogManager.getLogger(RadUnSealedAccountSyncTask.class);
    ...
    public void syncData() throws Exception {
        ...
        try {
            ...
            proc = conn.prepareCall("{call P_RAD_UNSERALED_ACCOUNT(?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
            getData(queryObject, conn, proc, endpoint);
        } catch (Exception e) {
            logger.info("同步非密封台账失败:" + e.getMessage());
            throw e;
        } finally {
            if (proc != null) {
                proc.close();
    ...
    private void getData(JSONObject queryObject, Connection conn, CallableStatement proc, String endpoint) throws Exception {
        ...
        if (dataArray != null && dataArray.size() > 0) {
            for (Object object : dataArray) {
                ...
                proc.setString(2, getString(tempObject.get("HSMC")));
                proc.setString(3, getString(tempObject.get("ENTERID")));
                proc.setString(4, getString(tempObject.get("QX")));
                proc.setString(5, getString(tempObject.get("XQSHR")));
                proc.setString(6, getString(tempObject.get("HD")));
                proc.setString(7, getString(tempObject.get("LY")));
                proc.setString(8, getString(tempObject.get("PKID")));
                proc.setString(9, getString(tempObject.get("UPDATETIME_RJWA")));
                proc.setString(10, getString(tempObject.get("SHRQ")));
                proc.setString(11, getString(tempObject.get("QXSHRQ")));
                proc.setString(12, getString(tempObject.get("YT")));
                proc.setString(13, getString(tempObject.get("BZ")));
                proc.setString(14, getString(tempObject.get("PC")));
                proc.addBatch();
            }
            proc.executeBatch();
            conn.commit();
